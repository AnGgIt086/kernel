...
#endif
/* BSP.Audio - 2022.03.28 - wangziru add for aw87xxx */
#ifdef CONFIG_SND_SOC_AW87XXX
extern bool mt6357_aw87xxx_chipid_ok(void);
extern int aw87xxx_set_profile(int dev_index, char *profile);
extern int aw87xxx_add_codec_controls(void *codec);
static int ext_spk_mode = 0;
static char *aw_profile[] = {"Music", "Receiver", "Off" , "Fm", "Voice"};
static const char *const spk_switch_text[] = {"None", "Spk", "Voice", "FM", "Rcv"};
...
#endif
/*end of BSP.Audio*/
/* BSP.Audio - 2022.05.17 - wangziru add for fs1599 */
#ifdef CONFIG_SND_SOC_FS1599
enum {
	FS1599_MUSIC_MODE = 0,
	FS1599_FM_MODE = 4,
	FS1599_OFF_MODE = 5,
	// vivo zhongzengeng add for FS1599_REC_MODE begin
	FS1599_REC_MODE = 15,
	FS1599_VOICE_MODE = 1,
	// vivo zhongzengeng add for FS1599_REC_MODE end
};
extern int mt6357_fs15xx_sence_load(int sence);
extern bool mt6357_fs15xx_chipid_ok(void);
#endif
/*end of BSP.Audio*/
...
static int headsetmicstatus;
static int mt6357_miso_drv = 0;
...
bool get_Headset_Mic_status(void)
{
	return headsetmicstatus;
}
EXPORT_SYMBOL(get_Headset_Mic_status);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	 ...
	 if (ucontrol->value.integer.value[0] == ext_spk_mode) {
		 pr_info("%s: the same value, no need to set.\n", __func__);
		 return 0;
	 }
	 ext_spk_mode = ucontrol->value.integer.value[0];

	 if (ext_spk_mode == AW87XXX_MUSIC_MODE)
	 {
		 if (mt6357_aw87xxx_chipid_ok()) {
			aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);
		 }
#if defined(CONFIG_SND_SOC_FS1599)
		 else if (mt6357_fs15xx_chipid_ok()) {
			 mt6357_fs15xx_sence_load(FS1599_MUSIC_MODE);
		 }
#endif
		 usleep_range(20*1000, 20*1001);
	 }
	 else if (ext_spk_mode == AW87XXX_VOICE_MODE)
	 {
		 if (mt6357_aw87xxx_chipid_ok()) {
			aw87xxx_set_profile(AW_DEV_0, aw_profile[4]);
		 }
#if defined(CONFIG_SND_SOC_FS1599)
		 else if (mt6357_fs15xx_chipid_ok()) {
			 // vivo zhongzengeng add for FS1599_VOICE_MODE begin
			 mt6357_fs15xx_sence_load(FS1599_VOICE_MODE);
			 // vivo zhongzengeng add for FS1599_VOICE_MODE end
		 }
#endif
		 usleep_range(20*1000, 20*1001);
	 }
	 else if (ext_spk_mode == AW87XXX_RCV_MODE)
	 {
		 if (mt6357_aw87xxx_chipid_ok()) {
			 aw87xxx_set_profile(AW_DEV_0, aw_profile[1]);
		 }
			 ...
			 // vivo zhongzengeng add for FS1599_REC_MODE begin
			 mt6357_fs15xx_sence_load(FS1599_REC_MODE);
			 // vivo zhongzengeng add for FS1599_REC_MODE end
         }
#endif
		 usleep_range(20*1000, 20*1001);
	 }
	 else if (ext_spk_mode == AW87XXX_FM_MODE)
	 {
		 if (mt6357_aw87xxx_chipid_ok()) {
			 aw87xxx_set_profile(AW_DEV_0, aw_profile[3]);
		 }
#if defined(CONFIG_SND_SOC_FS1599)
		 else if (mt6357_fs15xx_chipid_ok()) {
			 mt6357_fs15xx_sence_load(FS1599_FM_MODE);
         }
#endif
		 usleep_range(20*1000, 20*1001);
	 }
	 else
	 {
		 ...
		 if (mt6357_aw87xxx_chipid_ok()) {
			 ...
			 mt6357_fs15xx_sence_load(FS1599_OFF_MODE);
		 }
#endif
	 }
...
#ifdef CONFIG_SND_SOC_AW87XXX
	SOC_ENUM_EXT("vivo_ext_spk_mode", Audio_DL_Enum[15],
			vivo_codec_ext_spk_switch_get,
			vivo_codec_ext_spk_switch_set),
#endif
...
static void mt6357_codec_init_reg(struct snd_soc_component *component)
{
	...
	/* gpio miso driving set to default 4mA, 0x8888 */
	if (mt6357_miso_drv == 4) {
		pr_info("%s(), set miso_drv: %dmA\n",__func__, mt6357_miso_drv);
		Ana_Set_Reg(DRV_CON3, 0x8888, 0xffff);
	} else if (mt6357_miso_drv == 8) {
		pr_info("%s(), set miso_drv: %dmA\n",__func__, mt6357_miso_drv);
		Ana_Set_Reg(DRV_CON3, 0xaaaa, 0xffff);
	} else if (mt6357_miso_drv == 12) {
		pr_info("%s(), set miso_drv: %dmA\n",__func__, mt6357_miso_drv);
		Ana_Set_Reg(DRV_CON3, 0xcccc, 0xffff);
	} else if (mt6357_miso_drv == 16) {
		pr_info("%s(), set miso_drv: %dmA\n",__func__, mt6357_miso_drv);
		Ana_Set_Reg(DRV_CON3, 0xeeee, 0xffff);
	} else {
		pr_info("%s(), invalue miso_drv，set default 4mA\n",__func__);
		Ana_Set_Reg(DRV_CON3, 0x8888, 0xffff);
	}
...
static int mtk_codec_dev_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		res = of_property_read_u32(pdev->dev.of_node,
			"mt6357-miso-driving", &miso_drv);
		if (!res) {
			pr_debug("%s(), miso_drv: %d\n", __func__, miso_drv);
			mt6357_miso_drv = miso_drv;
		}
		...
		ret = of_property_read_u32(pdev->dev.of_node,
				     ...
				     &always_pull_down_enable);
		if (ret) {
			/* BSP.Audio modify begin by 72163315, 2022-09-23 */
			always_pull_down_enable = 1;
			dev_info(&pdev->dev,
				"%s(), get always_pull_down_enable fail, default 1\n",
				__func__);
			/* modify end of BSP.Audio */
		}
		/* check if enable always PullDown */
		ret = of_property_read_u32(pdev->dev.of_node,
				     "always_pull_low_off",
