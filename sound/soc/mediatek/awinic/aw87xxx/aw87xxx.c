 ...
 *
 ************************************************************************/
bool mt6357_aw87xxx_chipid_ok(void)
{
	return chipid_ok;
}
EXPORT_SYMBOL(mt6357_aw87xxx_chipid_ok);
					...
					const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev,
			kbuf[0],
			kbuf[1]);
					...
					size_t count, loff_t *pos)
{
	...
	for (i = 0; i < aw87xxx->aw_dev.reg_max_addr; i++) {
		if (!(aw87xxx->aw_dev.reg_access[i] & AW_DEV_REG_RD_ACCESS))
			continue;
		aw87xxx_dev_i2c_read_byte(&aw87xxx->aw_dev, i, &reg_val);
			  ...
			  const char *ubuf, size_t count)
{
	...
	aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, kbuf[0], kbuf[1]);
	kfree(temp);
