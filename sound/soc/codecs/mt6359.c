...
void changeFSM()
{
	pr_err("%s, mic record status %d, VOIP status %d, fast_charge_flag %d\n", \
	...
	/* Add by vivo audio xudong for compatible fast charge */
	if (mic_switch_flag == true && voip_switch_flag == false && fast_charge_flag)
		record_notify_status_to_charge(1);
	else
		record_notify_status_to_charge(0);
}
