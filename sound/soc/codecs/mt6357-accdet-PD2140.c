...
static void right_eint_work_callback(struct work_struct *work)
{
	...
	if (!hphl_val && !hphr_val) {
		...
		if (accdet->cable_type == NO_DEVICE) {
			hphl_val = gpio_get_value(accdet->r_gpiopin);
			hphr_val = gpio_get_value(accdet->r_gpiopin);
...
static inline void check_cable_type(void)
{
	...
	case MIC_BIAS:
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			mutex_lock(&accdet->res_lock);
			if (accdet->eint_sync_flag) {
				/* BSP.Audio modify for PLUG_OUT detect */
				//accdet->accdet_status = PLUG_OUT;
				if (accdet->accdet_no_work || accdet->cur_key != NO_KEY) {
					...
					pr_info("%s : BSP.Audio MIC_BIAS ACCDET_STATE_AB_11 headset_plug_out\n", __func__);
					headset_plug_out();
				}
				/* end of BSP.Audio modify */
			} else
				pr_notice("accdet hp has been plug-out\n");
			mutex_unlock(&accdet->res_lock);
		} else {
		...
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (accdet->eint_sync_flag) {
				...
				if (accdet->accdet_no_work || accdet->cur_key != NO_KEY) {
					...
					pr_info("%s : BSP.Audio HOOK_SWITCH ACCDET_STATE_AB_11 headset_plug_out\n", __func__);
					headset_plug_out();
				}
				/* end of BSP.Audio modify */
			} else
...
static int accdet_probe(struct platform_device *pdev)
{
	...
	/* BSP.Audio add for accdet bring up */
	accdet->r_gpiopin = -1;
	/* end of BSP.Audio add */
	accdet->base = 0;
