...
/* vivo audio add start */
char g_aw88xx_acf_name[AW_NAME_MAX]={0};
/* vivo audio add end */
		...
		struct device_node *np)
{
	...
	/* vivo audio add for load cfg start */
	const char *config = NULL;
	/* vivo audio add for load cfg end */
	...
	/* vivo audio add for load cfg start */
	ret = of_property_read_string(np, "vivo,aw882xx-acf", &config);
	if (config) {
		strcpy(g_aw88xx_acf_name, config);
	}
	/* vivo audio add for load cfg end */
...
/* vivo audio add for cali begin */
extern void set_vivo_cali_function(struct vivo_cali_ops *fun);
static struct aw_device *aw_pa_dev = NULL;
static struct smartpa_param_for_aw882xx vivo_spk_param;
static struct smartpa_param_for_aw882xx vivo_rcv_param;
static struct vivo_cali_ops aw882xx_cali_ops;
static int cali_channels;

/*
static int vivo_smartpa_calib_save(uint32_t *calib_value, int len)
{
	...
	pfile = filp_open(CALIBRATE_FILE, O_RDWR | O_CREAT, 0644); // TODO: error
	if (!IS_ERR(pfile)) {
		kernel_write(pfile, (char *)calib_value, len, &pos);
		filp_close(pfile, NULL);
		ret = 0;
	} else {
		pr_info("[SmartPA] %s, %s open failed\n", __func__, CALIBRATE_FILE);
		ret = -1;
	}
...
static int vivo_smartpa_calib_get(uint32_t *calib_value, int len)
{
	struct file *pfile = NULL;
	int found = 0;
	loff_t pos = 0;
	...
	if (IS_ERR_OR_NULL(pfile)) {
		pr_err("[SmartPA]%s, %s not found\n", __func__, CALIBRATE_FILE);
		found = 0;
	} else {
		found = 1;
		kernel_read(pfile, (char *)calib_value, len, &pos);
		filp_close(pfile, NULL);
	}
	return found;
}
...
static void parse_calib_to_param(struct smartpa_param_for_aw882xx *sp, uint32_t calib_re_mo)
{
...
static bool parse_q_1000_to_param(struct smartpa_param_for_aw882xx *sp, uint32_t q_1000)
{
	sp->Qts_1000 = q_1000;
...
static bool parse_f0_hz_to_param(struct smartpa_param_for_aw882xx *sp, uint32_t f0_hZ)
{
	sp->F0_hz = f0_hZ;
...
// smartpa calibration
int aw882xx_smartpa_init_dbg(char *buffer, int size) {
	int n = 0;
	struct smartpa_param_for_aw882xx *privPointer;
	int done = 0;
	...
	if(ret < 0){
		...
		//vivo audio add start, solve calibration probability fail problem
...
// return cali result
/* bit 0 is spk, bit 1 is rcv */
int aw882xx_smartpa_check_calib_dbg(void) {
	int ret = 0;
	int i;
	...
	for (i = 0; i < cali_channels; i++) {
		if (i == 0) {
		 ...
		 /* 0 is spk, 1 is rcv */
		ret = aw882xx_cali_read_re_from_nvram(&calib_mo_from_file[i], i);
		if (ret) {
...
//vivo audio add start, operating smartamp.bin file on user layer to solve gki unsymbol problem
int aw882xx_smartpa_init_calib_data(char *buffer, int size)
{
	pr_info("[SmartPA]%s, enter\n", __func__);
	return aw882xx_cali_init_smartamp_bin_data(buffer, size);
}

int aw882xx_smartpa_get_calib_data(char *buffer)
{
	pr_info("[SmartPA]%s, enter\n", __func__);
	return aw882xx_cali_get_smartamp_bin_data(buffer);
}
//vivo add end

//vivo audio add start for calib fail mute check
int aw882xx_smartpa_cali_fail_mute_enable(int en)
{
	pr_info("[SmartPA]%s, enable(%d) enter\n", __func__, en);
	return aw882xx_dev_set_cali_fail_mute_enable(en);
}
//vivo audio add end

// f0 and Q
int aw882xx_smartpa_read_freq_dbg(char *buffer, int size) {
	int ret = 0;
	int i, j;
	uint32_t calib_mo_from_file[4] = {0};
	int n = 0;
	...
	for (j = 0; j < cali_channels; j++) {
		if (j == 0) {
		 ...
		 /* second param is from dts: sound-channel */
		 /* 0 is spk, 1 is rcv */
		ret = aw882xx_cali_read_re_from_nvram(&calib_mo_from_file[j], j);
		if (ret) {
...
static void dump_param_info(struct smartpa_param_for_aw882xx *param) {
...
static int vivo_smartpa_parse_dt(struct i2c_client *i2c, struct smartpa_param_for_aw882xx *sp)
{
	unsigned int temp = 0;
	int ret = 0;
	pr_info("[SmartPA]%s, enter.\n", __func__);
...
/* vivo audio add for debug begin */
static ssize_t i2c_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (g_aw882xx_dev_cnt == 2) {
		n += scnprintf(buf, size, "SmartPA-stereo OK\n");
	} else if (g_aw882xx_dev_cnt == 1){
...
static ssize_t test_re_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	pr_info("[SmartPA]%s enter.\n", __func__);
	aw882xx_smartpa_init_dbg(test, size);
...
static ssize_t test_freq_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	pr_info("[SmartPA]%s enter.\n", __func__);
	aw882xx_smartpa_read_freq_dbg(test, 512);
	n += scnprintf(buf, size, test);
...
static int class_attr_create(struct aw882xx *aw882xx)
{
	...
	if (aw882xx == NULL) {
		pr_info("kobject_create_and_add audio-aw882xx file faild\n");
		return ret;
	}

	aw882xx->k_obj = kobject_create_and_add("audio-aw882xx", kernel_kobj);
	if (!aw882xx->k_obj) {
	...
	if (ret) {
		pr_info("%s, sysfs_create_group audio-aw882xx file faild\n", __func__);
	}
				...
				const struct i2c_device_id *id)
{
	...
	/* vivo audio add for bringup start */
	const char *vivo_type;
	/* vivo audio add for bringup end */

	aw_pr_info("enter addr=0x%x", i2c->addr);

	/* vivo audio add for bringup start */
	ret = dev_set_name(&i2c->dev, "%s-%x", "aw882xx", i2c->addr);
	if (ret || ERR_PTR(ret)) {
		aw_pr_info("dev_set_name failed\n");
		return ret;
	}
	/* vivo audio add for bringup end */
	...
	g_aw882xx_dev_cnt++;
	mutex_unlock(&g_aw882xx_lock);
	//mtk_spk_set_type(MTK_SPK_AW_AWINIC_SMARTPA_AW882xx);
	aw_dev_info(&i2c->dev, "dev_cnt %d", g_aw882xx_dev_cnt);
	/*vivo audio modified for bring up on 2021/12/27 start*/
	//mtk_spk_set_type(MTK_SPK_MEDIATEK_AW882XX);
	//vivo_set_codec_name(dev_name(&i2c->dev));
	/*vivo audio modified for bring up on 2021/12/27 end*/

	/* vivo audio add for cali begin */
	if(aw_pa_dev == NULL){
		aw_pa_dev = aw882xx->aw_pa;
		aw882xx_cali_ops.vivo_smartpa_init_dbg = aw882xx_smartpa_init_dbg;
		aw882xx_cali_ops.vivo_smartpa_read_freq_dbg = aw882xx_smartpa_read_freq_dbg;
		aw882xx_cali_ops.vivo_smartpa_check_calib_dbg = aw882xx_smartpa_check_calib_dbg;

		//vivo audio add start, operating smartamp.bin file on user layer to solve gki unsymbol problem
		aw882xx_cali_ops.vivo_smartpa_init_calib_data = aw882xx_smartpa_init_calib_data;
		aw882xx_cali_ops.vivo_smartpa_get_calib_data = aw882xx_smartpa_get_calib_data;
		//vivo audio add end

		//vivo audio add start for calib fail mute check
		aw882xx_cali_ops.vivo_smartpa_cali_fail_mute_enable = aw882xx_smartpa_cali_fail_mute_enable;
		//vivo audio add end

		//set_vivo_cali_function(&aw882xx_cali_ops);
		vivo_smartpa_debug_probe(&aw882xx_cali_ops);
