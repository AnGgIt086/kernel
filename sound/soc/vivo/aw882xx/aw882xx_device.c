...
static void aw_dev_mute(struct aw_device *aw_dev, bool mute)
{
    ...
    /* vivo audio add cali_fail_mute_en for calib */
	if (mute || ((aw_dev->cali_desc.cali_result == CALI_RESULT_ERROR) && cali_fail_mute_en)) {
...
//vivo audio add start for calib fail mute check
int aw882xx_dev_set_cali_fail_mute_enable(int en)
{
