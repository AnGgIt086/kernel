...
#define AW87XXX_DRIVER_VERSION	"v2.4.1"
...
static struct aw87xxx *aw87xxx_priv;
static const char *project_name = "default";
/* vivo audio add end */
/*************************************************************************
 * aw87xxx variable
 ************************************************************************/
...
struct aw87xxx *g_aw87xxx_390;

/* vivo audio add for set profile start */
static char *aw_profile[] = {"Music", "Receiver", "Voice", "Bypass", "Fm", "Off"};
static int g_aw_i2c_driver = 1;
/* vivo audio add for set profile end */
...
static int aw87xxx_pa_enable(int state)
{
	...
	case EXT_PA_SWICH_MUSIC:
		for (i = 0; i < g_aw87xxx_dev_cnt; i++) {
			pr_info("%s:enable dual pa %d\n",__func__, i);
			ret = aw87xxx_set_profile(i, aw_profile[0]);
		}
		break;
	case EXT_PA_SWICH_RECEIVER:
		ret = aw87xxx_set_profile(0, aw_profile[1]);
		break;
	case EXT_PA_SWICH_VOICE:
		for (i = 0; i < g_aw87xxx_dev_cnt; i++) {
			pr_info("%s:enable dual pa %d\n",__func__, i);
			ret = aw87xxx_set_profile(i, aw_profile[2]);
		}
		break;
	case EXT_PA_SWICH_BYPASS:
		for (i = 0; i < g_aw87xxx_dev_cnt; i++) {
			pr_info("%s:enable dual pa %d\n",__func__, i);
			ret = aw87xxx_set_profile(i, aw_profile[3]);
		}
		break;
	case EXT_PA_SWICH_FM:
		for (i = 0; i < g_aw87xxx_dev_cnt; i++) {
			pr_info("%s:enable dual pa %d\n",__func__, i);
			ret = aw87xxx_set_profile(i, aw_profile[4]);
		}
		break;
	case EXT_PA_SWICH_NONE:
		for (i = 0; i < g_aw87xxx_dev_cnt; i++) {
			pr_info("%s:disable pa %d\n",__func__, i);
			ret = aw87xxx_set_profile(i, aw_profile[5]);
		}
		break;
	default:
		for (i = 0; i < g_aw87xxx_dev_cnt; i++) {
			pr_info("%s:set default pa %d\n",__func__, i);
			ret = aw87xxx_set_profile(i, aw_profile[5]);
		}
		break;
	}
...
static void aw87xxx_fw_load_init(struct aw87xxx *aw87xxx, struct device_node *dev_node)
{
	int ret = -1;
	...
	/* vivo audio add for get project config name start */
	ret = of_property_read_string(dev_node, "vivo,aw87xxx-acf", &proj);
	if (ret == 0){
		AW_DEV_LOGI(aw87xxx->dev, "success find project-name:%s\n", proj);
		project_name = proj;	
	}else{
		project_name = AW87XXX_FW_BIN_NAME;
		AW_DEV_LOGI(aw87xxx->dev, "cannot find firmware_name property from dts,use default name:%s\n", project_name);
	}
	snprintf(aw87xxx->fw_name, AW87XXX_FW_NAME_MAX, "%s", project_name);
	/* vivo audio add for get project config name end */
	AW_DEV_LOGI(aw87xxx->dev,"AW87XXX_FW_NAME_MAX=%s\n",aw87xxx->fw_name);
			  ...
			  const char *ubuf, size_t count)
{
	...
	unsigned int kbuf[2];
	char *temp;
	struct aw87xxx *aw87xxx = g_aw87xxx_390;
				...
				const struct i2c_device_id *id)
{
	...
	/* cfg_load init */
	aw87xxx_fw_load_init(aw87xxx,dev_node);
	...
	vivo_set_codec_name(dev_name(&client->dev));
	//ljp add end
	/*add by yanghen for audio codec */
	mtk_spk_set_type(MTK_SPK_NOT_SMARTPA);
#ifdef BBK_IQOO_AUDIO_DEBUG
...
static int __init aw87xxx_pa_init(void)
{
	...
	if(g_aw_i2c_driver ==0 ){
		pr_info("%s 87xxx i2c_del_driver enter\n", __func__);
