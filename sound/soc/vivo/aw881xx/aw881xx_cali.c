...
static DEFINE_MUTEX(g_bin_data_lock);
static char *smartamp_bin_data = NULL;
//vivo add end
...
static int aw_fs_read(char *buf, size_t count, loff_t *pos)
{
	//vivo add start by linjinping, operating smartamp.bin file on user layer to solve gki unsymbol problem
	if ((*pos + count) > (AW_INT_DEC_DIGIT*AW_DEV_CH_MAX)) {
		pr_info("%s: invalid pos(%d) and count(%d)\n", __func__, *pos, count);
		return -EINVAL;
	}

	mutex_lock(&g_bin_data_lock);

	if (!smartamp_bin_data) {
		pr_info("%s: calib is not init\n", __func__);
		mutex_unlock(&g_bin_data_lock);
		return -EINVAL;
	}

	memcpy(buf, (smartamp_bin_data + *pos), count);
	mutex_unlock(&g_bin_data_lock);

	pr_info("%s: pos(%d), get calib re data: %s\n", __func__, *pos, buf);

	return 0;
	//vivo add end
}

static void aw_fs_write(char *buf, size_t count, loff_t *pos)
{
	//vivo add start by linjinping, operating smartamp.bin file on user layer to solve gki unsymbol problem
	if ((*pos + count) > (AW_INT_DEC_DIGIT*AW_DEV_CH_MAX)) {
		pr_info("%s: invalid pos(%d) and count(%d)\n", __func__, *pos, count);
	...
	mutex_lock(&g_bin_data_lock);
	if (!smartamp_bin_data) {
		smartamp_bin_data = kzalloc(AW_INT_DEC_DIGIT*AW_DEV_CH_MAX, GFP_ATOMIC);
		if (!smartamp_bin_data) {
			pr_info("%s: malloc failed\n", __func__);
			return;
		}
		memset(smartamp_bin_data, 0, AW_INT_DEC_DIGIT*AW_DEV_CH_MAX);
	}

	memcpy((smartamp_bin_data + *pos), buf,count);
	mutex_unlock(&g_bin_data_lock);

	pr_info("%s: pos(%d), write calib re data: %s\n", __func__, *pos, buf);
	//vivo add end
}
...
static int aw881xx_write_cali_re_to_file(uint32_t cali_re, uint8_t channel)
{
	...
	//vivo add start by linjinping, operating smartamp.bin file on user layer to solve gki unsymbol problem
	aw_fs_write(buf, strlen(buf), &pos);
	//vivo add end

	pr_info("%s: channel:%d buf:%s cali_re:%10d\n",
...
int aw881xx_get_cali_re_from_file(uint32_t *cali_re, uint8_t channel)
{
	/*struct inode *node;*/
	int f_size;
	char *buf = NULL;
	...
	//vivo add start by linjinping, operating smartamp.bin file on user layer to solve gki unsymbol problem
	if (aw_fs_read(buf, f_size, &pos)) {
		pr_err("%s: read cali re fail\n",  __func__);
		return -EINVAL;
	}
	//vivo add end
...
//vivo add start by linjinping, operating smartamp.bin file on user layer to solve gki unsymbol problem
int aw_cali_init_smartamp_bin_data(char *buf, size_t size)
{
	loff_t pos = 0;
	...
	if (size != AW_INT_DEC_DIGIT*AW_DEV_CH_MAX) {
		pr_info("%s, invalid size(%d)\n", __func__, size);
		return -1;
	}

	pr_info("%s: calib_re_data: %c%c%c%c, %c%c%c%c \n", __func__,
	         buf[6], buf[7], buf[8], buf[9],
	         buf[16], buf[17], buf[18], buf[19]
	       );

	aw_fs_write(buf, size, &pos);
...
int aw_cali_get_smartamp_bin_data(char *buf)
{
	loff_t pos = 0;
	...
	memcpy(buf,"new_tas:",strlen("new_tas:"));
	aw_fs_read(buf+strlen("new_tas:"), AW_INT_DEC_DIGIT*AW_DEV_CH_MAX, &pos);

	pr_info("%s, success \n", __func__);

	return AW_INT_DEC_DIGIT*AW_DEV_CH_MAX;
}
//vivo add end
...
{
	aw_dev_dbg(aw881xx->dev, "%s: enter, cali_check_st=%d, cali_result=%d\n",
		       __func__, aw881xx->cali_desc.cali_check_st, cali_result);
...
int aw881xx_get_cali_re(struct aw_cali_desc *cali_desc)
{
	...
	ret = aw881xx_get_cali_re_from_phone(aw881xx, &cali_re);
	if (ret < 0) {
		cali_desc->re = AW_ERRO_CALI_VALUE;
	...
	show_re = AW_DSP_RE_TO_SHOW_RE(cali_re);
	if ((show_re < aw881xx->re_range.re_min) || (show_re > aw881xx->re_range.re_max)) {
	...
	cali_desc->re = cali_re;
	cali_desc->cali_re = show_re;
...
static int aw881xx_set_cali_re_to_phone(struct aw881xx *aw881xx)
{
	...
	return aw881xx_write_cali_re_to_file(cali_desc->re,
					aw881xx->channel);
#else
	return 0;
#endif
...
static int aw_cali_store_cali_re(struct aw881xx *aw881xx, int32_t re)
{
	...
	if ((re > aw881xx->re_range.re_min) && (re < aw881xx->re_range.re_max)) {
		aw881xx->cali_desc.re = AW_SHOW_RE_TO_DSP_RE(re);
		ret = aw881xx_set_cali_re_to_phone(aw881xx);
		if (ret < 0) {
...
int aw_cali_svc_get_dev_cali_val(struct aw881xx *aw881xx, int type, uint32_t *data_buf)
{
	switch (type) {
	case GET_RE_TYPE:
...
static int aw_cali_svc_get_smooth_cali_re(struct aw881xx *aw881xx)
{
	...
	cali_desc->re = re_average - ra_val;
	...
	aw_dev_info(aw881xx->dev, "%s: re[%d]mohm", __func__, cali_desc->cali_re);
				...
				const char *buf, size_t count)
{
	...
	cali_desc->re = AW_SHOW_RE_TO_DSP_RE(databuf[0]);
				...
				struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			"cali_re=%umohm\n",
			(uint32_t)(AW_DSP_RE_TO_SHOW_RE(cali_attr->cali_re)));
				...
				struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			"re=%umohm\n", AW_DSP_RE_TO_SHOW_RE(cali_desc->re));
...
int aw_cali_svc_cali_cmd(struct aw881xx *aw881xx, int cali_cmd, bool is_single, unsigned int flag)
{
	switch (cali_cmd) {
		case AW_CALI_CMD_RE : {
