...
#include "sipa_aux_dev_if.h" //vivo audio add
...
/* vivo audio add start */
static bool sia_chipid_ok = false;
static unsigned int g_sia8159_dev_cnt = 0;
static sipa_dev_t *sia8159_priv;
static sipa_dev_t *sia8159_priv_vivo[2];
/* vivo audio add end */
...
void distinguish_chip_type(sipa_dev_t *si_pa)
{
	...
	if (CHIP_TYPE_UNKNOWN == si_pa->chip_type ||
		0 != check_sipa_status(si_pa)) {  // high --sipa
	...
	struct sipa_dev_s *si_pa)
{
	...
	pr_info("%s: si_pa->chip_type = %d,si_pa->name:%s si_pa->channel_num:%d running \r\n",
		 __func__, si_pa->chip_type, si_pa->name,si_pa->channel_num);
	...
	char *buf)
{
	...
	if(si_pa->chip_type == CHIP_TYPE_UNKNOWN){
		pr_info("[ info][%s] %s: chip type unknown, return \r\n", LOG_FLAG, __func__);
		return -ENOENT;
	}
...
/* vivo audio add for set pa profile start */
static int sipa_power_set_vivo(int state, sipa_dev_t *si_pa, int scene)
{
	struct sipa_dev_s *si_pa_vivo = si_pa;

	pr_info("%s: enter, state: %d, pa_cnt:%d, scene:%d\n", __func__, state, g_sia8159_dev_cnt, scene);
	...
	if (1 == state) {
		...
		sipa_resume(si_pa_vivo);
		si_pa->sipa_on = true;
		pr_info("%s:dxl: power on pa:%d, scene:%d\n", __func__, si_pa_vivo->channel_num, scene);
	} else {
		si_pa->scene = scene;
		sipa_suspend(si_pa_vivo);
		si_pa->sipa_on = false;
		if (sipa_is_chip_en(si_pa)) {
			sipa_reboot(si_pa);
		}
		pr_info("%s:dxl: power off pa:%d, scene:%d\n", __func__, si_pa_vivo->channel_num, si_pa->scene);
	}

	return 0;
}

static int sipa_pa_enable(int state)
{
	int ret = 0;
	int i = 0;

	pr_info("%s: enter, state: %d, pa_cnt:%d\n",
		__func__, state, g_sia8159_dev_cnt);

	switch (state) {
	case EXT_PA_SWICH_MUSIC:
		for (i = 0; i < g_sia8159_dev_cnt; i++) {
			ret = sipa_power_set_vivo(1, sia8159_priv_vivo[i], 0);
		}
		break;
	case EXT_PA_SWICH_RECEIVER:
		ret = sipa_power_set_vivo(1, sia8159_priv_vivo[0], 2);
		ret = sipa_power_set_vivo(0, sia8159_priv_vivo[1], 0);
		break;
	case EXT_PA_SWICH_VOICE:
		for (i = 0; i < g_sia8159_dev_cnt; i++) {
			ret = sipa_power_set_vivo(1, sia8159_priv_vivo[i], 1);
		}
		break;
	case EXT_PA_SWICH_BYPASS:
		for (i = 0; i < g_sia8159_dev_cnt; i++) {
			ret = sipa_power_set_vivo(1, sia8159_priv_vivo[i], 3);
		}
		break;
	...
	case EXT_PA_SWICH_NONE:
		for (i = 0; i < g_sia8159_dev_cnt; i++) {
			ret = sipa_power_set_vivo(0, sia8159_priv_vivo[i], 0);
		}
		break;
	default:
		for (i = 0; i < g_sia8159_dev_cnt; i++) {
...
/* vivo audio add for set pa profile end */
...
/* vivo audio add start */
#ifdef SYS_NODESIA
static ssize_t sia_i2c_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	sipa_dev_t *si_pa = sia8159_priv;
	const int size = 40;
	int n = 0;

	pr_info("[SmartPA-%d]%s enter.\n", __LINE__, __func__);

	if (g_sia8159_dev_cnt == 2) {
		n += scnprintf(buf, size, "SmartPA-stereo OK %d\n",
				get_smartpa_i2c_check());
	} else {
		n += scnprintf(buf, size, "SmartPA-0x%x %s\n", si_pa->client->addr,
				sia_chipid_ok ? "OK" : "ERROR");
	}
	return n;
}

static struct kobj_attribute dev_attr_i2c =
	__ATTR(i2c, 0664, sia_i2c_show, NULL);

static struct attribute *sys_node_attributes[] = {
	&dev_attr_i2c.attr,
	NULL
};
static struct attribute_group sia8159_node_attribute_group = {
	.name = NULL,		/* put in device directory */
	.attrs = sys_node_attributes
};

static int class_attr_create(sipa_dev_t *si_pa)
{
	int ret = -1;
	...
	si_pa->k_obj = kobject_create_and_add("audio-smartpa", kernel_kobj);
	if (!si_pa->k_obj) {
		pr_info("%s: sipa kobject_create_and_add audio-smartpa file faild\n", __func__);
		return 0;
	}

	ret = sysfs_create_group(si_pa->k_obj, &sia8159_node_attribute_group);
	if (ret) {
...
static int class_attr_remove(sipa_dev_t *si_pa)
{
	if (si_pa != NULL && si_pa->k_obj) {
		sysfs_remove_group(si_pa->k_obj, &sia8159_node_attribute_group);
		kobject_del(si_pa->k_obj);
		si_pa->k_obj = NULL;
	}
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	int addr_offset = 0;
	/* vivo audio add start */
	struct vivo_codec_function *vivo_codec_function_sipa = NULL;
	/* vivo audio add end */
	...
	/* vivo audio add start */
	vivo_codec_function_sipa = get_vivo_codec_function();
	if (!vivo_codec_function_sipa) {
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("[  err][%s] %s: i2c_check_functionality return -ENODEV \r\n",
			LOG_FLAG, __func__);
		ret = -ENODEV;
		goto exit_check_functionality_failed; //vivo audio add
	}
	...
	sipa_compatible_chips_adapt(si_pa);
	/* vivo audio add for return start */
	if(si_pa->chip_type == CHIP_TYPE_UNKNOWN){
		pr_info("[ info][%s] %s: chip type unknown, return \r\n", LOG_FLAG, __func__);
		sia_chipid_ok = false;
		device_remove_file(&si_pa->pdev->dev, &dev_attr_sipa_cmd);
		ret = -ENOMEM;
		goto exit_check_functionality_failed; //vivo audio add
	}
	release_smartpa_lock(); //vivo audio add
	/* vivo audio add for return end */
	...
	/* vivo audio add for set pa profile start */
	if(g_sia8159_dev_cnt==0){
		set_smartpa_i2c_check(1);
		if (!sia8159_priv_vivo[0])
			sia8159_priv_vivo[0] = si_pa;
	}
	else if(g_sia8159_dev_cnt==1){
		set_smartpa_i2c_check(2);
		if (!sia8159_priv_vivo[1])
			sia8159_priv_vivo[1] = si_pa;
	}
	/* vivo audio add for set pa profile end */
	...
	sia_chipid_ok=true;
#ifdef SYS_NODESIA
	class_attr_create(si_pa);
#endif
	if (vivo_codec_function_sipa) {
		pr_info("%s:vivo_codec_function malloc success, set sipa_pa_enable function\n", __func__);
		vivo_codec_function_sipa->ext_pa_enable = sipa_pa_enable;
	}
	/* vivo audio add end */
