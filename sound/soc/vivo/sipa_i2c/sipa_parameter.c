...
void sipa_param_load_fw(struct device *dev)
{
	...
	/* vivo audio add for pa cfg file start */
	const char *proj = NULL;
	int ret = 0;
	/* vivo audio add for pa cfg file end */
	...
	/* vivo audio add for pa cfg file start */
	ret = of_property_read_string(si_pa->client->dev.of_node, "vivo,si_pa_cfg", &proj);
	if (ret == 0){
		pr_info("[debug][%s] %s: success find project-name:%s\n", LOG_FLAG, __func__, proj);
		sipa_fw_name = proj;
	}else{
		pr_err("[  err][%s] %s: cannot find firmware_name property from dts,use default name:%s\n", LOG_FLAG, __func__, sipa_fw_name);
	}
	/* vivo audio add for pa cfg file end */
