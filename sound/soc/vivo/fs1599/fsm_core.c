...
void fsm_shutdown(fsm_dev_t *fsm_dev)
{
	fsm_config_t *cfg = fsm_get_config();
	if (!fsm_dev) {
		return;
...
void fsm_speaker_onn(void)
{
	...
	pr_debug("done");
	fsm_mutex_unlock();
	fsm_read_status_reg(1); //add by yanghen for pa start dump reg
}
...
void fsm_speaker_off(void)
{
	...
	fsm_read_status_reg(0); //add by yanghen for pa end dump reg
	fsm_mutex_lock();
