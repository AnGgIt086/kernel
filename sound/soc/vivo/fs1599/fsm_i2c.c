...
// add by yanghen for dump reg 
static struct kobject *fsmxxx_kobj;
static fsm_dev_t *g_fsm_dev;
/* customize configrature */
#include "fsm_firmware.c"
#include "fsm_misc.c"
#include "fsm_codec.c"
// add by yanghen for dump reg start
/**********************************************************
 * fsm attribute
***********************************************************/
...
#define FSM_REG_MAX         0xF0

const unsigned char fsm_reg_show[FSM_REG_MAX] = {
	[0x00] = REG_RD_ACCESS,
	[0x01] = REG_RD_ACCESS,
	[0x03] = REG_RD_ACCESS,
	[0x04] = REG_RD_ACCESS,
		...
		struct kobj_attribute *attr, char *buffer)
{
	...
	int ret;
	int i;
	ret = fsm_access_key(g_fsm_dev, 1);
	for (i = 0; i < FSM_REG_MAX; i++) {
		if (fsm_reg_show[i] & REG_RD_ACCESS) {
			fsm_i2c_reg_read(g_fsm_dev, i, &reg_val);
			len += snprintf(buffer+len, PAGE_SIZE-len,
					"reg:0x%02x=0x%04x\n", i, reg_val);
		}
	}
	ret |= fsm_access_key(g_fsm_dev, 0);
	return len;
}

static struct kobj_attribute dev_attr_reg =
	__ATTR(reg, 0664, reg_show, NULL);
...
static int class_attr_create(struct kobject *kobj)
{
 	...
 	int ret = -1;
  	...
  	scnprintf(name, 48, "audio-fsmxxx");
  	...
  	pr_info("%s: sysfs create %s successful\n", __func__, name);
  	return ret;
}
...
// add by yanghen for dump reg end
 
void fsm_read_status_reg(int enable)
{
	uint16_t reg_val = 0;
	int ret;
	int i;
	...
	for (i = 0; i < FSM_REG_MAX; i++) {
		if (fsm_reg_show[i] & REG_RD_ACCESS) {
			fsm_i2c_reg_read(g_fsm_dev, i, &reg_val);
			pr_info("fsm pa status %s reg:0x%x,val= 0x%x",enable?"open":"close",i,reg_val);
		}
	}
	ret |= fsm_access_key(g_fsm_dev, 0);
} 
// add by yanghen for dump reg end
static int fs1599n_pa_enable(int state)
{
	int ret = 0;
	...
	case EXT_PA_SWICH_VOICE:
		  fsm_set_scene(FS_PA_VOICE);
		  fsm_speaker_onn(); //add by yanghen for start pa 
		  break;
	case EXT_PA_SWICH_MUSIC:
		  fsm_set_scene(FS_PA_MUSIC);
		  fsm_speaker_onn(); //add by yanghen for start pa 
		  break;
	case EXT_PA_SWICH_FM:
		  fsm_set_scene(FS_PA_FM);
		  fsm_speaker_onn(); //add by yanghen for start pa 
		  break;
			...
			const struct i2c_device_id *id)
{
	...
	vivo_set_codec_name(FSM_DRV_NAME);
	//ljp add end

	dev_info(&i2c->dev, "i2c probe completed");
	// yanghen add for fsm init
	fsm_init();
	vivo_codec_function->ext_pa_enable = fs1599n_pa_enable;
	//yanghen add for dump reg
	g_fsm_dev = fsm_dev;
	class_attr_create(fsmxxx_kobj);
	return 0;
}
