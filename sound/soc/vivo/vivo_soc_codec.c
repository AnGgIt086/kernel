...
#ifdef CONFIG_VIVO_CODEC_FS1599N
#include "fs1599/fsm_public.h"
#endif
#ifdef CONFIG_VIVO_CODEC_FS1519N
#include "fs1519n/fs15xx.h"
#endif
#ifdef pr_debug
#undef pr_debug
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	struct vivo_codec_prv *vivo_codec = snd_soc_component_get_drvdata(component);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (vivo_codec->shdnpin < 0) {
		pr_err("%s: this GPIO is no invalid\n", __func__);
		return 0;
	}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((vivo_codec->ext_spk_mode == EXT_PA_SWICH_MUSIC) ||
		(vivo_codec->ext_spk_mode == EXT_PA_SWICH_RECEIVER) ||
		(vivo_codec->ext_spk_mode == EXT_PA_SWICH_VOICE) ||
		(vivo_codec->ext_spk_mode == EXT_PA_SWICH_BYPASS) ||
		(vivo_codec->ext_spk_mode == EXT_PA_SWICH_FM) ||
		(vivo_codec->ext_spk_mode == EXT_PA_SWICH_NONE)) {
...
static int vivo_soc_codec_probe(struct snd_soc_component *codec)
{
	pr_err("%s: enter\n", __func__);
	snd_soc_component_set_drvdata(codec, vivo_codec);
	//vivo_codec->codec = codec;
	snd_soc_add_component_controls(codec, vivo_soc_codec_controls,
				   ARRAY_SIZE(vivo_soc_codec_controls));
	/*Add by yanghen vivo audio for fs1599n pa*/
#ifdef CONFIG_VIVO_CODEC_FS1599N
	fsm_add_platform_controls(codec); 
#endif
#ifdef CONFIG_VIVO_CODEC_FS1519N
	 fs15xx_add_codec_kcontrol(codec);
#endif
	return 0;
}

static void vivo_soc_codec_remove(struct snd_soc_component *component)
{
...
static const struct snd_soc_component_driver vivo_soc_component_drv = {
...
static int vivo_soc_platform_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(&pdev->dev, &vivo_soc_component_drv,
