...
/* vivo audio add start */
static sipa_dev_t *sia81xx_priv;
/* vivo audio add end */
...
static int sipa_power_set_vivo(int state, sipa_dev_t *si_pa, int scene)
{
	...
	pr_info("%s: enter, state: %d, scene:%d\n", __func__, state, scene);
...
static int sipa_pa_enable(int state)
{
	...
	case EXT_PA_SWICH_MUSIC:
		ret = sipa_power_set_vivo(1, sia81xx_priv, 0);
		break;
	case EXT_PA_SWICH_RECEIVER:
		ret = sipa_power_set_vivo(1, sia81xx_priv, 2);
		break;
	case EXT_PA_SWICH_VOICE:
		ret = sipa_power_set_vivo(1, sia81xx_priv, 1);
		break;
	case EXT_PA_SWICH_BYPASS:
		ret = sipa_power_set_vivo(1, sia81xx_priv, 3);
		break;
	case EXT_PA_SWICH_FM:
		ret = sipa_power_set_vivo(1, sia81xx_priv, 0);
		break;
	case EXT_PA_SWICH_NONE:
			ret = sipa_power_set_vivo(0, sia81xx_priv, 0);
		break;
	default:
			ret = sipa_power_set_vivo(0, sia81xx_priv, 0);
		break;
...
static ssize_t sia_i2c_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	n += scnprintf(buf, size, "SmartPA-mono OK,is sia8102\n");
	return n;
}
...
static struct attribute_group sia81xx_node_attribute_group = {
...
static int class_attr_create(sipa_dev_t *si_pa)
{
	...
	ret = sysfs_create_group(si_pa->k_obj, &sia81xx_node_attribute_group);
	if (ret) {
...
static int class_attr_remove(sipa_dev_t *si_pa)
{
	if (si_pa != NULL && si_pa->k_obj) {
		sysfs_remove_group(si_pa->k_obj, &sia81xx_node_attribute_group);
