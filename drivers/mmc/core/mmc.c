...
static void mmc_select_driver_type(struct mmc_card *card)
{
	...
	p = (((u32)card->ext_csd.raw_sectors[3]<<24)
	 + ((u32)card->ext_csd.raw_sectors[2]<<16)
	 + ((u32)card->ext_csd.raw_sectors[1]<<8)
	 + ((u32)card->ext_csd.raw_sectors[0]));

	mega = p/1024/1024*512;

	if (mega > 128*1024) {
		size_m = 256;
	} else if (mega > 64*1024) { //if memory is larger than 64G
		size_m = 128; // It should be 128G
	} else if (mega > 32*1024) {  //larger than 32G
		size_m = 64; //should be 64G
	} else if (mega > 16*1024) {
		size_m = 32;
	} else if (mega > 8*1024) {
		size_m = 16;
	} else if (mega > 6*1024) {
		...
		size_m = 6;  //RAM may be 6G
	} else if (mega > 3*1024) {
		size_m = 4;
	} else if (mega > 2*1024) {
		size_m = 3; //RAM may be 3G
	} else if (mega > 1024) {
		size_m = 2;
	} else if (mega > 512) {
		size_m = 1;
	...
	if (fixed_drv_type >= 0)
		...
		drive_strength = mmc_select_drive_strength(card, card->ext_csd.hs200_max_dtr,
	...
	struct mmc_card *oldcard)
{
	...
	} else if ( !card->ext_csd.man_bkops_en) {
		if (!card->ext_csd.auto_bkops_en) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
