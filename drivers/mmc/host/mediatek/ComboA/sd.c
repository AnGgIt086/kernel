...
static ssize_t msdc_attr_##name##_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)\
{ \
	fmt_type tmp;\
	struct mmc_host *mmc = dev_get_drvdata(dev);\
	struct msdc_host *host = mmc_priv(mmc);\
...
static void msdc_add_device_attrs(struct msdc_host *host, struct device_attribute *attrs[])
{
	...
	for (i = 0; attrs[i]; ++i) {
		ret = device_create_file(host->pdev, attrs[i]);
		if (ret)
			dev_err(host->pdev, "failed to register attribute: %s; err=%d\n",
...
static void msdc_remove_device_attrs(struct msdc_host *host, struct device_attribute *attrs[])
{
	...
	if (!attrs)
		return;
	for (i = 0; attrs[i]; ++i)
		device_remove_file(host->pdev, attrs[i]);
}
...
static void msdc_card_reset(struct mmc_host *mmc)
{
	...
	pr_notice("overtime %d,reset card\n", overtime);
