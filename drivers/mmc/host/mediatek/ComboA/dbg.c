...
/* bsp-memory add emmc_size node for test - start */
static int msdc_emmc_size_show(struct seq_file *m, void *v)
{
	int r_size_m = 0;
	int in_mega = 0;
	int k = 0;
	...
	k = (((u32)card->ext_csd.raw_sectors[3]<<24)
	 + ((u32)card->ext_csd.raw_sectors[2]<<16)
	 + ((u32)card->ext_csd.raw_sectors[1]<<8)
	 + ((u32)card->ext_csd.raw_sectors[0]));

	in_mega = k/1024/1024*512;

	if (in_mega > 128*1024) {
		r_size_m = 256;
	} else if (in_mega > 64*1024) { //if memory is larger than 64G
		r_size_m = 128; // It should be 128G
	} else if (in_mega > 32*1024) {  //larger than 32G
		r_size_m = 64; //should be 64G
		...
		r_size_m = 6;  //RAM may be 6G
	} else if (in_mega > 3*1024) {
		r_size_m = 4;
	} else if (in_mega > 2*1024) {
		r_size_m = 3; //RAM may be 3G
...
static int msdc_emmc_size_open(struct inode *inode, struct file *file)
{
	return single_open(file, msdc_emmc_size_show, inode->i_private);
}

static const struct file_operations emmc_size_fops = {
	.open = msdc_emmc_size_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
/* bsp-memory add emmc_size node for test - end */
