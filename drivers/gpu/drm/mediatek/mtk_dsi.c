	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & DYNFPS_DSI_MIPI_CLK) {
...
#ifdef CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO
		if ((fps_dst == 60) || (fps_dst == 90)) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SET_AOD_OFF, 0);
		}
		...
		if (fps_dst == 30) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SWITCH_30HZ, 0);
		}
		else if ((fps_dst == 60) || (fps_dst == 90)) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SWITCH_30TO60OR90HZ, 0);
		}
#else
		mtk_dsi_porch_setting(comp, handle, DSI_HFP, dsi->hfp_byte);
...
#ifdef CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO
		if (fps_dst == 60) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SWITCH_60HZ, 0);
		} else if (fps_dst == 90) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SWITCH_90HZ, 0);
		}
#endif
		mtk_dsi_porch_setting(comp, handle, DSI_VFP, vfp);
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_SET_BL:
	{
		...
		break;
	case DSI_SET_HDR_FLAG:
	{
		struct mtk_dsi *dsi =
			container_of(comp, struct mtk_dsi, ddp_comp);
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (panel_ext && panel_ext->funcs
			&& panel_ext->funcs->vivo_update_hdr_flag)
			panel_ext->funcs->vivo_update_hdr_flag(dsi,
				mipi_dsi_dcs_write_gce,
