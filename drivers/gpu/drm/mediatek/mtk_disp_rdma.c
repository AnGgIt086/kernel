				 ...
				 struct cmdq_pkt *handle)
{
	...
	unsigned int val = 0;
#if defined(CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO) || defined(CONFIG_DRM_PANEL_PD2215_SAMSUNG_S6E3FC3_FHDP_CMD)
	...
	if (mtk_crtc) {
		...
		if (state && state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
			...
			DDPMSG("%s GS_RDMA_TH_LOW_FOR_SODI is in max: 0x%x\n", __func__,
				gs[GS_RDMA_TH_LOW_FOR_SODI]);
		} else {
			DDPINFO("%s GS_RDMA_TH_LOW_FOR_SODI is in nor: 0x%x\n", __func__,
				gs[GS_RDMA_TH_LOW_FOR_SODI]);
		}
	}
#endif
