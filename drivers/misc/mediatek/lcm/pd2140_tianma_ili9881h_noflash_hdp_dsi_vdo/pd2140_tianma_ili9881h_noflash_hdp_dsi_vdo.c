...
static struct LCM_setting_table init_setting_12bit[] = {
	...
	{0xFF, 3, {0x98,0x81,0x01} }, //mura modify
	{0x0E, 1, {0x0A} },
	{0x0F, 1, {0x0A} },
	{0x12, 1, {0x0A} },
	{0xB8, 1, {0x00} },
	...
	{0xFF, 3, {0x98,0x81,0x05} }, //TS VGLO&VGL Setting
	{0xCC, 1, {0x8D} },
	{0xCD, 1, {0x8D} },
	{0xCE, 1, {0x8D} },
	{0xCF, 1, {0x8D} },
	...
	{0xFF, 3, {0x98,0x81,0x06} }, //OSC + 1%,64.64mHZ
	{0x13, 1, {0xEB} },
	{0x14, 1, {0x41} },
	{0x15, 1, {0x98} },
	{0x16, 1, {0x41} },

	{0xFF, 3, {0x98,0x81,0x06} },  //internal vporch
	{0x27, 1, {0xFF} },
	{0xFF, 3, {0x98,0x81,0x02} },
	{0x06, 1, {0x90} },

	{0xFF, 3, {0x98, 0x81, 0x0B} }, //D3G EN
	{0x80, 1, {0x01} },
...
static struct LCM_setting_table lcm_cmd_cabc_level1[] = {
	...
	{0xFF, 3, {0x98, 0x81, 0x00} },
	{0x55, 1, {0x01} },*/
	{0xFF, 3, {0x98, 0x81, 0x03} },//turn off cabc
	{0xAA, 1, {0xDC} },
	{0xFF, 3, {0x98, 0x81, 0x00} },
	{0x55, 1, {0x00} },
};
...
{
	LCM_INFO("%s tianma_ili9881h -> enter power on begin, panel_reset_state=%d\n", __func__, panel_reset_state);
	...
	************************************/
		...
		LCM_INFO("%s pd2140_tianma_ili9881h_noflash_hdp_dsi_vdo -> enter power on end\n", __func__);
	}
}
...
{
	LCM_INFO("[LCM]tianma ili9881h- init begin\n");
	lcm_reset_setting(1);
	MDELAY(10);
	lcm_reset_setting(0);
	MDELAY(3);
	...
	******************************************************************/
	...
	LCM_INFO("[LCM]ili9881h- init end\n");
}
...
static void lcm_suspend(void)
{
	push_table(lcm_suspend_setting,
		...
		1);
	panel_off_state = 1;
	LCM_INFO("[LCM]ili9881h- lcm_suspend display off\n");
}
...
static void lcm_reset_for_touch(void)
{
	...
	MDELAY(12);
	panel_reset_state = 1;
	LCM_INFO("[LCM]ili9881h- lcm_reset_for_touch end\n");
}
...
struct LCM_DRIVER pd2140_tianma_ili9881h_noflash_hdp_dsi_vdo_lcm_drv = {
	.name = "pd2140_tianma_ili9881h_noflash_hdp_dsi_vdo",
