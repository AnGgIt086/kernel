...
{
	LCM_INFO("%s txd_nt36525a -> enter power on begin, panel_reset_state=%d\n", __func__, panel_reset_state);
	...
	************************************/
		...
		LCM_INFO("%s txd_nt36525a -> enter power on end\n", __func__);
	}
}
...
static void lcm_suspend_power(void)
{
	...
	lcm_enp_setting(0);
	MDELAY(5);
	LCM_INFO("[LCM]lcm_suspend_power txd_nt36525a- vddi keep high level\n");
	panel_reset_state = 0;// clear reset state
}

static void lcm_init(void)
{
	LCM_INFO("[LCM]txd_nt36525a- init begin\n");
	lcm_reset_setting(1);
	MDELAY(3);
	lcm_reset_setting(0);
	MDELAY(3);
	...
	******************************************************************/
	...
	LCM_INFO("[LCM]txd_nt36525a- init end\n");
}
...
static void lcm_suspend(void)
{
	push_table(lcm_suspend_setting,
		...
		1);
	panel_off_state = 1;
	LCM_INFO("[LCM]txd_nt36525a- lcm_suspend display off\n");
}
...
static void lcm_reset_for_touch(void)
{
	...
	MDELAY(12);
	panel_reset_state = 1;
	LCM_INFO("[LCM]txd_nt36525a- lcm_reset_for_touch end\n");
}
...
struct LCM_DRIVER pd2140_txd_nt36525a_noflash_hdp_dsi_vdo_lcm_drv = {
	.name = "pd2140_txd_nt36525a_noflash_hdp_dsi_vdo",
