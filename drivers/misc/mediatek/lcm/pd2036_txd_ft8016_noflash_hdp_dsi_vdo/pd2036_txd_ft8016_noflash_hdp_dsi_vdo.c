...
static struct LCM_setting_table init_setting_12bit[] = {
	...
	{0x90,16,{0x00,0x24,0x42,0x0A,0xE3,0x91,0xA4,0xF0,0xAF,0x5C,0x69,0x20,0x69,0x81,0x26,0x00}},
	...
	{0x41,2,{0x5A,0x0A}},
	{0x80,16,{0xFF,0x84,0x8F,0x03,0x00,0x0E,0x1A,0x29,0x3A,0x4B,0x51,0x85,0x5E,0x9B,0x98,0x5F}},
	{0x90,16,{0x89,0x5A,0x56,0x45,0x34,0x24,0x16,0x0B,0x00,0x0E,0x1A,0x29,0x3A,0x4B,0x51,0x85}},
	{0xA0,13,{0x5E,0x9B,0x98,0x5F,0x89,0x5A,0x56,0x45,0x34,0x24,0x16,0x0B,0x00}},
	{0x41,2,{0x5A,0x0B}},
	...
	{0x90,16,{0xF3,0x00,0x20,0x4B,0x85,0xAA,0xD5,0x06,0x91,0x91,0x57,0xA1,0x6A,0xB5,0x41,0x64}},
	...
	{0x41,2,{0x5A,0x19}},
	{0x80,16,{0xD6,0xD0,0xBC,0xB5,0xA7,0xA2,0x99,0x93,0x8F,0x8A,0x86,0x82,0x77,0xF7,0xFF,0xFF}},
	{0x90,16,{0xFF,0xFF,0xFF,0xBF,0x7F,0x3F,0x0F,0xFF,0xAF,0xB5,0x71,0x0E,0x6C,0x4A,0x69,0x08}},
	{0xA0,4,{0x00,0x00,0x46,0x00}},
	{0x41,2,{0x5A,0x1A}},
	{0x80,16,{0x00,0x04,0x08,0x0C,0x00,0x10,0x14,0x18,0x1C,0x00,0x20,0x27,0x2F,0x37,0xFC,0x3F}},
	{0x90,16,{0x47,0x4F,0x57,0xAB,0x5F,0x67,0x6F,0x77,0x05,0x7E,0x86,0x8E,0x96,0x5B,0x9E,0xA5}},
	{0xA0,16,{0xAD,0xB5,0x6C,0xBD,0xC4,0xCC,0xD4,0x18,0xDB,0xE3,0xEB,0xF2,0x87,0xF6,0xF8,0xF9}},
	{0xB0,16,{0x01,0x00,0x04,0x08,0x0C,0x00,0x10,0x14,0x18,0x1C,0x00,0x1F,0x27,0x2F,0x38,0x3F}},
	{0xC0,16,{0x40,0x48,0x50,0x57,0xD5,0x60,0x68,0x70,0x77,0xD4,0x7F,0x87,0x90,0x97,0x8F,0x9F}},
	{0xD0,16,{0xA7,0xAF,0xB7,0xFF,0xBF,0xC7,0xD0,0xD8,0x0F,0xE0,0xE8,0xF0,0xF8,0x00,0xFC,0xFE}},
	{0xE0,16,{0xFF,0x00,0x00,0x03,0x07,0x0B,0xBC,0x0F,0x13,0x17,0x1B,0x06,0x1E,0x26,0x2E,0x35}},
	{0xF0,16,{0xCB,0x3D,0x45,0x4C,0x54,0x62,0x5C,0x63,0x6B,0x73,0x1C,0x7A,0x82,0x8A,0x91,0xC7}},
	{0x41,2,{0x5A,0x1B}},
	{0x80,16,{0x99,0xA1,0xA9,0xB1,0x16,0xB8,0xC0,0xC8,0xD0,0xAB,0xD8,0xE0,0xE8,0xF1,0x39,0xF5}},
	{0x90,4,{0xF7,0xF8,0x25,0x00}},
	{0x41,2,{0x5A,0x20}},
	...
	{0x90,12,{0xFF,0x0F,0x00,0x00,0x00,0x03,0x00,0x00,0x00,0xF7,0x07,0x35}},
	{0x41,2,{0x5A,0x24}},
	{0x80,16,{0x00,0x03,0x00,0xFF,0xFF,0x20,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xB8,0xBC}},
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_frontporch = 90;  //36
...
/*#ifdef CONFIG_MTK_LCM_MIPI_CLK_CONTROL*/
	if (rf_hw_id == 1) {
		...
		LCM_INFO("%s PD2036 rf_hw_id PLL_CLOCK %d\n", __func__, params->dsi.PLL_CLOCK);
	}
/*#endif*/
...
{
	LCM_INFO("%s ft8016 -> enter power on begin, panel_reset_state=%d\n", __func__, panel_reset_state);
	...
	************************************/
		...
		LCM_INFO("%s ft8016 -> enter power on end\n", __func__);
	}
}
...
static void lcm_suspend(void)
{
	if ((!strcmp(project_name, "PD2139F_EX")) || (!strcmp(project_name, "PD2139F_EX_SC"))) {
		push_table(lcm_suspend_setting2,
			sizeof(lcm_suspend_setting2) / sizeof(struct LCM_setting_table),
			1);
	} else {
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	level >>= 1;
	if (lcm_id_version == 0x03 && level >= 1900) {
		level = 1900;
	}
	/*level =level*16;*/
...
struct LCM_DRIVER pd2036_txd_ft8016_noflash_hdp_dsi_vdo_lcm_drv = {
	.name = "pd2036_txd_ft8016_noflash_hdp_dsi_vdo",
