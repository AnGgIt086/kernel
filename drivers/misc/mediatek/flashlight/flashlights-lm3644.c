...
/* add for PD2140 factory flash */
static char node_one_buf[20] = {"0"};

#define AW3644_Device_id 		0x02
#define AW36413_Device_id 		0x12
#define AW3644_slave_address 	0x63
#define AW36413_slave_address 	0x6B
#endif

#if defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
static char node_one_buf[20] = {"0"};
#define AW3644_AW36515_Device_id 		0x02
#define AW3644_AW36515_slave_address	0x63
#define AW36515_Chip_id	0x30
#define AW3644_Chip_id	0x36
#endif
...
/*LM3644's duty is used by AW3644*/
static const int lm3644_current[LM3644_LEVEL_NUM] = {  /*current:mA */
	 33,  51,  65,  75,  100, 120, 140, 198,  246, 304,
	351, 398, 445, 503, 550, 597, 656, 703,  750, 796,
	857, 902, 961, 1008, 1067, 1100};
...
static const int lm3644_current[LM3644_LEVEL_NUM] = {  /*current:mA */
	...
	351, 398, 445, 503, 550, 597, 656, 703,  750, 796,
	857, 902, 961, 1008, 1067, 1100};
static const unsigned char aw36515_torch_level[LM3644_LEVEL_NUM] = {
	0x10, 0x19, 0x20, 0x25, 0x32, 0x3C, 0x46, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
static const unsigned char lm3644_torch_level[LM3644_LEVEL_NUM] = {
	0x0A, 0x11, 0x15, 0x19, 0x22, 0x27, 0x2F, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
static const unsigned char aw36515_flash_level[LM3644_LEVEL_NUM] = {
	0x03, 0x06, 0x07, 0x09, 0x0C, 0x0E, 0x11, 0x18, 0x1E, 0x26,
	0x2C, 0x32, 0x38, 0x3f, 0x45, 0x4b, 0x53, 0x59, 0x5F, 0x65,
	0x6C, 0x72, 0x7A, 0x80, 0x87, 0x8B};
static const unsigned char lm3644_flash_level[LM3644_LEVEL_NUM] = {
	0x01, 0x03, 0x05, 0x07, 0x09, 0x0B, 0x0D, 0x10, 0x14, 0x19,
	0x1D, 0x21, 0x25, 0x2A, 0x2E, 0x32, 0x37, 0x3B, 0x3F, 0x43,
	0x48, 0x4C, 0x50, 0x54, 0x59, 0x5D};
...
static int lm3644_read_reg(struct i2c_client *client, u8 reg)
{
	...
	pr_debug("flashlight i2c read [0x%02x] at reg [0x%02x], ret = %d ", val, reg, val);
...
#if defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
	if(AW3644_AW36515_Device_id == device_id && AW36515_Chip_id == chip_id)
		val = aw36515_torch_level[level];
	else
		val = lm3644_torch_level[level];
#else
	if (chip_id == 0x36 || device_id == 0x12)
...
#elif defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
		if(AW3644_AW36515_Device_id == device_id && AW36515_Chip_id == chip_id)
			lm3644_write_reg(lm3644_i2c_client, LM3644_REG_TORCH_LEVEL_LED1, 0x32);
		else if(AW3644_AW36515_Device_id == device_id && AW3644_Chip_id == chip_id)
			lm3644_write_reg(lm3644_i2c_client, LM3644_REG_TORCH_LEVEL_LED1, 0x22);
		else
			lm3644_write_reg(lm3644_i2c_client, LM3644_REG_TORCH_LEVEL_LED1, 0x22);
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
		if (fl_arg->arg > -1 && fl_arg->arg < LM3644_LEVEL_NUM)
			pr_info("FLASH_IOC_SET_DUTY(%d): %d  lm3644_current[fl_arg->arg] =%d device_id=[0x%x]\n",
					channel, (int)fl_arg->arg, lm3644_current[(int)fl_arg->arg], device_id);
		else
			pr_err("FLASH_IOC_SET_DUTY(%d): %d  device_id=[0x%x]\n",
					channel, (int)fl_arg->arg, device_id);
		lm3644_set_level(channel, fl_arg->arg);
#elif defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
		if (fl_arg->arg > -1 && fl_arg->arg < LM3644_LEVEL_NUM)
			pr_info("FLASH_IOC_SET_DUTY(%d): %d  lm3644_current[fl_arg->arg] =%d device_id=[0x%x] chip_id=[0x%x]\n",
					channel, (int)fl_arg->arg, lm3644_current[(int)fl_arg->arg], device_id,chip_id);
		else
			pr_err("FLASH_IOC_SET_DUTY(%d): %d  device_id=[0x%x] chip_id=[0x%x]\n",
					channel, (int)fl_arg->arg, device_id,chip_id);
		lm3644_set_level(channel, fl_arg->arg);
#else
	...
	case FLASH_IOC_SET_ONOFF:
		...
		if (fl_arg->arg == 1) {
			...
			if (lock_touch == 0 && lock_touch_sub == 0) {
				...
				//PD2140 also need this code to fix current problem
...
{
	/* Move to set driver for saving power */
	pr_info("release state_pin_short from [%d] to [%d]", state_pin_short, 0);
	state_pin_short = 0;
	return 0;
}
...
static int lm3644_set_driver(int set)
{
	...
	if (set) {
		#if defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX) || defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
		pr_info("Set driver before delay use_count: %d", use_count);
		msleep(20);
		pr_info("Set driver after  delay use_count: %d", use_count);
		#endif
		if (!use_count)
			ret = lm3644_init();
		use_count++;
...
{
	pr_debug("echo lm3644_FLASH debug buf, %s ", buf);
	sprintf(node_one_buf, "%s\n", buf);

	if ((strcmp("0", buf) == 0) || (strcmp("0\x0a", buf) == 0)) {
		pr_debug("lm3644_FLASH  0");
		lm3644_disable(LM3644_CHANNEL_CH1);
		lm3644_set_driver(0);
	} else {
		pr_debug("lm3644_FLASH  1");
		lm3644_set_driver(1);
		lm3644_set_level_ch1(0);
		lm3644_timeout_ms[LM3644_CHANNEL_CH1] = 0;
		lm3644_enable(LM3644_CHANNEL_CH1);
	}
...
static DEVICE_ATTR(lm3644_FLASH, 0664, att_show, att_store);
#endif
...
static int lm3644_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("lm3644_i2c_probe lm3644 CHIP_ID = 0x%x\n", chip_id);
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
	if (device_id != AW36413_Device_id) {
		lm3644_i2c_client->addr = AW3644_slave_address;
		device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
		pr_info("lm3644_i2c_probe aw3644 DEVICE_ID = 0x%x\n", device_id);
	}
#elif defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
	if (device_id != AW3644_AW36515_Device_id) {
			lm3644_i2c_client->addr = AW3644_AW36515_slave_address;
			device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
			chip_id = lm3644_read_reg(lm3644_i2c_client, LM3644_CHIP_ID);
			pr_info("lm3644_i2c_probe flashlight DEVICE_ID = 0x%x CHIP_ID = 0x\n", device_id,chip_id);
	}

#elif defined(CONFIG_MTK_CAM_PD2074F_EX)
    lm3644_i2c_client->addr = 0x6B;  //20210301 lishuo add  Compatible with LM3644
	device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
	//pr_debug("lm3644_i2c_probe lm3644 DEVICE_ID = %d\n", device_id);
	if(device_id<0){  //20210301 lishuo modify  Compatible with LM3644 
		lm3644_i2c_client->addr = 0x63; //20210301 lishuo modify  Compatible with LM3644 
		device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
		pr_info("lm3644_i2c_probe lm36413 DEVICE_ID = %d\n", device_id);
	}
#else
	if(device_id  != 0x02){
...
#if defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX) || defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
//add for PD2140 factory flash
	sysfs_create_file(&dev->dev.kobj, &dev_attr_lm3644_FLASH.attr);
#endif
	pr_debug("Probe done.\n");
