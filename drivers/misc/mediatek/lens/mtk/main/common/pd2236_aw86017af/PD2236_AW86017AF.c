...
#include "lens_info.h"
#include "PD2236_AW86017AF.h"

#define AW86017AF_SLAVE_ADDR		(0x18) /* 0x0c: 0x18 >> 1 */
#define AW86017AF_DRIVER_VERSION	"v1.0.0"
 ...
 * I2c read/write
 ******************************************************************************/
static int AW86017AF_WriteRegs(unsigned char a_uAddr, unsigned char *a_uData,
							unsigned int len)
{
	int ret = 0;
	...
	g_pstAF_I2Cclient->addr = (AW86017AF_SLAVE_ADDR >> 1);
...
static int AW86017AF_ReadRegs(unsigned char a_uAddr, unsigned char *a_puData,
...
static inline int AW86017AF_SoftReset(void)
{
	uint8_t val = 0;
	int ret = 0;

	val = AW_SHUTDOWN;
	ret = AW86017AF_WriteRegs(AW_REG_CONTROL, &val, AW_DATA_BYTE_1);
	if (ret < 0) {
...
static inline int AW86017AF_CfgCurrentGears(unsigned char chip_id)
{
	uint8_t gears = 0;
	...
	if (chip_id == AW86017_CHIPID) {
		...
		AW86017AF_WriteRegs(AW_REG_IMAX, &gears, AW_DATA_BYTE_1);
	} else {
...
static inline int AW86017AF_SetAlgoMode(void)
{
	...
	/* enter advance mode */
	ret = AW86017AF_WriteRegs(mode_cmd[0], &mode_cmd[1], AW_DATA_BYTE_1);
	if (ret < 0) {
	...
	/* Identification IC */
	ret = AW86017AF_ReadRegs(AW_REG_CHIP_ID, &chip_id, AW_DATA_BYTE_1);
	if (ret < 0) {
	...
	if (type == AW_BOTTOM_MOTOR) {
		algo_cfg[0] = (AW86017_RING << 7) | algo_mode;
		algo_cfg[1] = div;
		algo_cfg[2] = vrct;

		ret = AW86017AF_WriteRegs(AW_REG_ALG_MODE, algo_cfg,
								AW_DATA_BYTE_3);
		if (ret < 0) {
		...
		ret = AW86017AF_ReadRegs(AW_REG_CONTROL, &ctl_val,
								AW_DATA_BYTE_1);
		if (ret < 0) {
		...
		ret = AW86017AF_WriteRegs(AW_REG_CONTROL, &ctl_val,
								AW_DATA_BYTE_1);
		if (ret < 0) {
	...
	AW86017AF_CfgCurrentGears(chip_id);
#endif
	g_motor_type = type;
...
static inline int AW86017AF_InitDrv(void)
{
	int ret = 0;
...
static int AW86017AF_SetPos(unsigned long a_u4Position)
{
	int ret = 0;
	...
	g_pstAF_I2Cclient->addr = (AW86017AF_SLAVE_ADDR >> 1); /* 0x18 >> 1 */
...
static inline int AW86017AF_MoveVCM(unsigned long a_u4Position)
{
	AW_LOGI("Start.");
	...
	if (*g_pAF_Opened == 2) {
		if (AW86017AF_SetPos(a_u4Position) == 0) {
			spin_lock(g_pAF_SpinLock);
...
static inline int AW86017AF_SetMacro(unsigned long a_u4Param)
{
	AW_LOGI("Start.");
...
static inline int AW86017AF_SetInf(unsigned long a_u4Param)
{
	AW_LOGI("Start.");
...
static inline int AW86017AF_GetVCMInfo(
...
long PD2236_AW86017AF_Ioctl_Main(struct file *a_pstFile, unsigned int a_u4Command,
							unsigned long a_u4Param)
{
	...
	case AFIOC_G_MOTORINFO:
		ret = AW86017AF_GetVCMInfo(
				(__user struct stAF_MotorInfo *) (a_u4Param));
		break;
	case AFIOC_T_MOVETO:
		ret = AW86017AF_MoveVCM(a_u4Param);
		break;
	case AFIOC_T_SETMACROPOS:
		ret = AW86017AF_SetMacro(a_u4Param);
		break;
	case AFIOC_T_SETINFPOS:
		ret = AW86017AF_SetInf(a_u4Param);
		break;
	default:
		AW_LOGE("NO CMD!.");
		ret = -EPERM;
...
int PD2236_AW86017AF_Release_Main(struct inode *a_pInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		AW_LOGE("Wait\n");
		AW86017AF_SetPos(265);
		msleep(10);
		AW86017AF_SetPos(230);
		msleep(10);
		AW86017AF_SetPos(195);
		msleep(10);
		AW86017AF_SetPos(160);
		msleep(10);
	}
...
int AW86017AF_PowerDown(struct i2c_client *pstAF_I2Cclient, int *pAF_Opened)
{
	...
	/* enter PD mode, PD = 1; */
	ret = AW86017AF_WriteRegs(pd_cmd[0], &pd_cmd[1], AW_DATA_BYTE_1);
	if (ret < 0) {
...
int PD2236_AW86017AF_SetI2Cclient_Main(struct i2c_client *pstAF_I2Cclient,
...
int PD2236_AW86017AF_GetFileName_Main(uint8_t *pFileName)
{
...
MODULE_DESCRIPTION("AW86017AF VCM Driver");
