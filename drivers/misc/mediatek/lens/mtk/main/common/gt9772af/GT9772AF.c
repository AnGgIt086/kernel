...
static long g_i4Dir;
static int g_sr = 3;
static unsigned short g_af_i2c_slave_addr = AF_I2C_SLAVE_ADDR;
...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
	...
	g_pstAF_I2Cclient->addr = g_af_i2c_slave_addr;
...
static int gt9772_init(void)
{
	...
	LOG_INF("[GT9772AF] Get IC Info and Check IIC Address Begin\n");
	puSendCmd[0] = 0x00;
	g_pstAF_I2Cclient->addr = 0x1C;
	g_pstAF_I2Cclient->addr = g_pstAF_I2Cclient->addr >> 1;
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 1);
	if (i4RetValue == 1) {
		g_af_i2c_slave_addr = 0x1C;
		i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, recv_buf, 1);
		if (i4RetValue == 1) {
			LOG_INF("[GT9772AF] Get IC info: 0x%x form iic addr 0x1C, datasheet: 0xEE", recv_buf[0]);
		}
	} else {
		LOG_INF("[GT9772AF] I2C address != 0x1C\n");
	}
	puSendCmd[0] = 0x00;
	...
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 1);
	if (i4RetValue == 1) {
		g_af_i2c_slave_addr = 0x18;
		i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, recv_buf, 1);
		if (i4RetValue == 1) {
			LOG_INF("[GT9772AF] Get IC info: 0x%x form iic addr 0x18, datasheet: 0xEE", recv_buf[0]);
		}
	} else {
		LOG_INF("[GT9772AF] I2C address != 0x18\n");
	}
	LOG_INF("[GT9772AF] Get IC Info and Check IIC Address End\n");
