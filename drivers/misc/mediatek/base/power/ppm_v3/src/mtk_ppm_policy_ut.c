...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX)
static int ppm_ut_fix_freq_cts_idx_proc_show(struct seq_file *m, void *v)
{
	int i;
...
#define CTS_MAGIC_STRING      "cts_freq_idx"

static ssize_t ppm_ut_fix_freq_cts_idx_proc_write(struct file *file,
	const char __user *buffer, size_t count, loff_t *pos)
{
	int i = 0;
	int *fix_freq;
	...
	while ((tok = strsep(&tmp, " ")) != NULL) {
        ...
        if (i == 0) {
            if (strcmp(tok, CTS_MAGIC_STRING) != 0) {
                ppm_err("Invalid input: %s\n", tok);
			    goto out;
            } else {
                i++;
		...
		if (kstrtoint(tok, 10, &fix_freq[i - 1])) {
			ppm_err("Invalid input: %s\n", tok);
			goto out;
		} else
			i++;
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX)
PROC_FOPS_RW(ut_fix_freq_cts_idx);
#endif
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX)
	const struct pentry cts_entry[] = {
		PROC_ENTRY(ut_fix_freq_cts_idx),
	};
#endif
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX)
	if (!proc_create(cts_entry[0].name, 0666,
		policy_dir, cts_entry[0].fops)) {
		ppm_err("%s(), create /proc/%s failed\n",
			__func__, cts_entry[0].name);
		ret = -EINVAL;
		goto out;
	}
#endif
