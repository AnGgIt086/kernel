...
#if defined CONFIG_VIVO_SENSOR_SCP_RESET_ENABLE
...
static bool send_lcmid_success;
...
static void send_lcm_softid_to_hub(void)
{
	...
	//pr_err("scp_reset,set lcm_soft_id:0x%x\n", lcm_soft_id);
	if (lcm_soft_id != 0 && lcm_soft_id != 0xff) {
		send_lcmid_success = true;
		cmd_args[0] = SENSOR_COMMAND_LIGHT_PARAM_SET_LCM_ID;
		cmd_args[1] = lcm_soft_id;
		err = sensor_set_vivo_cmd_to_hub(ID_LIGHT, cmd_args, ARRAY_SIZE(cmd_args));
		pr_err("scp_reset,set lcm_soft_id:0x%x\n", lcm_soft_id);
		if (err < 0)
			pr_err("scp reset, SENSOR_COMMAND_LIGHT_PARAM_SET_LCM_ID fail!\n");
	} else {
		pr_err("get lcm id failed\n");
	}
...
/* add by vsen team start */
	...
	if (ps_cali_data != -1) {
		...
		err = sensor_set_vivo_cmd_to_hub(ID_PROXIMITY, cmd_args, ARRAY_SIZE(cmd_args));
		pr_err("scp_reset,set ps_cali:%d\n", ps_cali_data);
		if (err < 0)
			pr_err("scp reset, SENSOR_COMMAND_PROX_SET_ENG_CALI_DATA fail!\n");
	}
	if (ps_hardware_broken != -1) {
		cmd_args[0] = SENSOR_COMMAND_PROX_NOTIFY_PS_BROKEN;
		cmd_args[1] = ps_hardware_broken;
		err = sensor_set_vivo_cmd_to_hub(ID_PROXIMITY, cmd_args, ARRAY_SIZE(cmd_args));
		pr_err("scp_reset,set hardware_broken:%d\n", ps_hardware_broken);
		if (err < 0)
			pr_err("scp reset, SENSOR_COMMAND_PROX_NOTIFY_PS_BROKEN fail!\n");
	}
#endif
/* add by vsen team end */
...
/*add by vsen team begin*/
static int alspshub_factory_do_vivo_command(uint8_t sensorType, int32_t *args, int args_len)
{
	int err = 0;
	vivo_sensor_eng_cmd cmd = args[0];
	if (cmd == SENSOR_COMMAND_LIGHT_BOOST_NOTIRY) {
		err = sensor_batch_to_hub(ID_LIGHT, 0, *(args + 1), *(args + 2));
		pr_err("do_vivo_command (cmd)0x%x (val1)%d (val2)%d err(%d)\n", cmd, *(args + 1), *(args + 2), err);
		return 0;
	}
	...
	if (cmd == SENSOR_COMMAND_PROX_SET_ENG_CALI_DATA) {
		ps_cali_data = *(args + 1);
		pr_err("do_vivo_command save ps_cali_data:%d\n", ps_cali_data);
	}
	if (cmd == SENSOR_COMMAND_PROX_NOTIFY_PS_BROKEN) {
		ps_hardware_broken = *(args + 1);
		pr_err("do_vivo_command save ps_hardware_broken:%d\n", ps_hardware_broken);
	}
#endif
...
#if defined CONFIG_VIVO_SENSOR_SCP_RESET_ENABLE
	if (cmd == SENSOR_COMMAND_PROX_NOTIFY_TEMP_CALI) {
		if ((int)*(args + 1) == -1)
			ps_hardware_broken = 1;
		else
			ps_hardware_broken = 0;
		pr_info("tempcali done,set hardware_broken:%d\n", ps_hardware_broken);
	}
#endif
	return 0;
}
...
static struct alsps_factory_fops alspshub_factory_fops = {
	...
	/*add by vsen team begin*/
	.do_vivo_commands = alspshub_factory_do_vivo_command,
	/*add by vsen team end*/
};
...
static int als_enable_nodata(int en)
{
	...
	/*get lcm id failed, try again*/
	if (send_lcmid_success == false) {
		send_lcm_softid_to_hub();
	}
