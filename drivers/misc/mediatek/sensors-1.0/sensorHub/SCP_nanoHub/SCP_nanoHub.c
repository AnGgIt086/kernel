...
int sensor_set_vivo_cmd_to_hub(uint8_t sensorType, int32_t *args, int args_len)
{
	/* Notice: data size should be VIVO_SENSOR_COMMAND_ARGS_SIZE */
	union SCP_SENSOR_HUB_DATA req;
	int len = 0, err = 0, index = 0;
	pr_err("%s sensorType %d !!!\n", __func__, sensorType);
    printk("cyk-----2---1-------\n");
	if (args_len > VIVO_SENSOR_COMMAND_ARGS_SIZE) {
	...
	req.set_cust_req.vivoCmds.action = CUST_ACTION_VIVO_COMMAND;
	/*Data[0] will the command*/
	for (index = 0; index < args_len; index++)
		req.set_cust_req.vivoCmds.data[index] = *(args + index);

	len = offsetof(struct SCP_SENSOR_HUB_SET_CUST_REQ, custData)
				+ sizeof(req.set_cust_req.vivoCmds);
	err = scp_sensorHub_req_send(&req, &len, 1);
	if (err == 0) {
		...
		if (req.set_cust_rsp.vivoCmds.action != CUST_ACTION_VIVO_COMMAND) {
			pr_err("scp_sensorHub_req_send failed!(%d)\n", req.set_cust_rsp.vivoCmds.action);
			return -1;
		}
		pr_info("%s:(CMD)0x%x (val)%d val2(%d)\n", __func__,
			req.set_cust_rsp.vivoCmds.data[0], req.set_cust_rsp.vivoCmds.data[1],
			req.set_cust_rsp.vivoCmds.data[2]);
		for (index = 0; index < args_len; index++)
			*(args + index) = req.set_cust_rsp.vivoCmds.data[index];
