...
{
	struct sarhub_ipi_data *obj = obj_ipi_data;
#if defined(CONFIG_VIVO_CHARGING_FOR_2140)
		int32_t value[7] = {0};  //7
		int err = 0;
	...
	if (event->flush_action == FLUSH_ACTION)
		...
		printk("2140 SAR_IOCTL_READ_SENSORDATA %d %d %d %d %d %d %d %d %d %d\n", cmd_args[1], cmd_args[2], cmd_args[3], cmd_args[4], cmd_args[5], cmd_args[6], cmd_args[7], cmd_args[8], cmd_args[9],err);
#else
		int32_t value[6] = {0};
		int err = 0;
	...
	if (event->flush_action == FLUSH_ACTION)
		...
		value[2] = event->sar_event.data[2];
		err = sar_data_report_t(value, (int64_t)event->time_stamp);
		printk("2074 SAR_IOCTL_READ_SENSORDATA %d %d %d %d \n",value[0],value[1],value[2],err);
#endif
