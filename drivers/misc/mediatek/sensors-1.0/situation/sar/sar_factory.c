					...
					unsigned long arg)
{
	...
	printk("---cyk---sar_factory_unlocked_ioctl cmd:%d\n", cmd);
	...
	case SAR_IOCTL_READ_SENSORDATA:
		printk("---cyk---SAR_IOCTL_READ_SENSORDATA\n");
		if (sar_factory.fops != NULL &&
			sar_factory.fops->do_vivo_commands != NULL) {
			cmd_args[0] = SENSOR_COMMAND_SAR_GET_DIFF_OFFSET;
			err = sar_factory.fops->do_vivo_commands(ID_SAR, cmd_args, ARRAY_SIZE(cmd_args));
			if (err < 0) {
			...
			printk("----cyk---SAR_IOCTL_READ_SENSORDATA %d %d %d %d %d %d %d %d %d\n", cmd_args[1], cmd_args[2], cmd_args[3],
	...
	case SAR_IOCTL_ENABLE_CALI: /*compensation cali*/
		...
		if (sar_factory.fops != NULL &&
			...
			err = sar_factory.fops->do_vivo_commands(ID_SAR, cmd_args, sizeof(cmd_args)/sizeof(cmd_args[0]));
			if (err < 0) {
