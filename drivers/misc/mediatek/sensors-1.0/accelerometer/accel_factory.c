				       ...
				       unsigned long arg)
{
	...
	int data = 0;
	uint32_t cmd_args[VIVO_SENSOR_COMMAND_ARGS_SIZE] = {0, 0};
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		if (accel_factory.fops != NULL && accel_factory.fops->do_vivo_commands != NULL) {
			cmd_args[0] = SENSOR_COMMAND_ACCEL_SET_ENG_CALI_DATA;
			err = accel_factory.fops->do_vivo_commands(ID_ACCELEROMETER, cmd_args, ARRAY_SIZE(cmd_args));
			if (err < 0) {
	...
	case GSENSOR_IOCTL_CLR_INT:
		if (accel_factory.fops != NULL && accel_factory.fops->do_vivo_commands != NULL) {
			...
			err = accel_factory.fops->do_vivo_commands(ID_ACCELEROMETER,
