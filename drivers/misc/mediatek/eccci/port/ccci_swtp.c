...
const struct of_device_id swtp_of_match[] = {
	...
	{ .compatible = SWTP0_COMPATIBLE_DEVICE_ID, },
...
static bool HQ_swtp_cmdline(void)
{
	char *tempbuf = NULL;
	char board_state[] = "androidboot.swtp=";
	char ptr[1];
	tempbuf = strstr(saved_command_line,board_state);
	if (tempbuf)
        {
		printk("[niko3]tempbuf is not null\n");
		strncpy(ptr, tempbuf + 17, 1);
        }
	if ((int)ptr[0] == 49) {
		printk("[niko1]false ptr[0]=%d \n", ptr[0]);
		return false;
	} else {
		printk("[niko2]false ptr[0]=%d \n", ptr[0]);
		return true;
        }
}
...
int swtp_init(int md_id)
{
	...
	if (HQ_swtp_cmdline()) {
		node = of_find_matching_node(NULL, &swtp_of_match[1]);
	} else {
		node = of_find_matching_node(NULL, &swtp_of_match[0]);
	}
	if (md_id < 0 || md_id >= SWTP_MAX_SUPPORT_MD) {
