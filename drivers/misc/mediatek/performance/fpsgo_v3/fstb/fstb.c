...
static int fstb_enable, fstb_active, fstb_active_dbncd, fstb_idle_cnt;//vivo mhy add patch for rear video start
		...
		unsigned int cur_max_freq, u64 ulID)
{
	...
	if (!fstb_active)//vivo mhy add patch for rear video
		fstb_active = 1;
...
int fpsgo_fbt2fstb_update_cpu_frame_info(
		...
		long long Runnging_time,//vivo mhy add patch for rear video
...
//vivo mhy add patch for rear video end
		...
		goto out;//vivo mhy add patch for rear video
...
out://vivo mhy add patch for rear video
	mtk_fstb_dprintk(
	...
	if (mid && cpu_time_ns >= 0) {//vivo mhy add patch for rear video
...
//vivo mhy add patch for rear video end
	...
	if (iter == NULL) {
		...
		new_frame_info->render_idle_cnt = 0;//vivo mhy add patch for rear video
...
static int fstb_get_queue_fps1(struct FSTB_FRAME_INFO *iter,
		long long interval, int *is_fps_update)//vivo mhy add patch for rear video
{
	...
	unsigned long long retval = 0;
	int frame_count = 0;//vivo mhy add patch for rear video
	...
	for (j = i + 1; j < iter->queue_time_end; j++) {
		...
		frame_count++;//vivo mhy add patch for rear video
	}

	*is_fps_update = frame_count ? 1 : 0;//vivo mhy add patch for rear video
...
//vivo mhy add patch for rear video end
	...
	return retval;//vivo mhy add patch for rear video
}

static int fps_update(struct FSTB_FRAME_INFO *iter)
{
	int is_fps_update = 0;//vivo mhy add patch for rear video

	iter->queue_fps =
		fstb_get_queue_fps1(iter, FRAME_TIME_WINDOW_SIZE_US, &is_fps_update);//vivo mhy add patch for rear video

	return is_fps_update;//vivo mhy add patch for rear video
}
...
//vivo mhy add patch for rear video end
			...
			iter->render_idle_cnt = 0;//vivo mhy add patch for rear video
...
//vivo mhy add patch for rear video end
	...
	if (fstb_enable && fstb_active_dbncd)//vivo mhy add patch for rear video
		enable_fstb_timer();
	else
		disable_fstb_timer();
