...
#if defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
static ssize_t dvfsrc_cts_force_vcore_dvfs_opp_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int val = 0;
	struct mtk_dvfsrc *dvfsrc = dev_get_drvdata(dev);
...
static DEVICE_ATTR_WO(dvfsrc_cts_force_vcore_dvfs_opp);
...
#endif
#if defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
	&dev_attr_dvfsrc_cts_force_vcore_dvfs_opp.attr,
#endif
	NULL,
};
