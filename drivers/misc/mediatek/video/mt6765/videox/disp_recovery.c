...
extern unsigned int vivo_esd_check_ps_status;
extern int vts_node_reset_fw_download(enum vts_type type);
...
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		...
		do {
			/*******************************************************
			only pd1901 ft8006P need it to reload initial code.
			when para of 0xED  is 0xAA ,lcd_need_reset =1, we need to reload
			********************************************************/
			if (!strcmp(project_name, "PD1901") &&
				(lcm_software_id == 0x60 || lcm_software_id == 0x63 || lcm_software_id == 0x74|| lcm_software_id == 0x78|| lcm_software_id == 0x6f|| lcm_software_id == 0x70)) {
				ret = lcd_need_reset;
				lcd_need_reset = 0;
				} else {
			...
			if ((!strcmp(project_name, "PD2139F_EX")) || ((!strcmp(project_name, "PD2139F_EX_SC")))) {
				if (lcm_software_id == 0x07) {
					vts_node_reset_fw_download(VTS_TYPE_MAIN);
				}
			}
