...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.custom1 = {
		.pclk = 144000000, /*record different mode's pclk*/
		.linelength  = 1932, /*record different mode's linelength*/
		.framelength = 3088, /*record different mode's framelength*/
		.startx = 0, /*record different mode's startx of grabwindow*/
		.starty = 0, /*record different mode's starty of grabwindow*/
...
extern int vivo_main3_otp_read_ov8856(void);
int vivo_main3_otp_read_when_power_on_ov8856;
extern otp_error_code_t MAIN3_OTP_ERROR_CODE_OV8856;
...
{
	ov8856_table_write_cmos_sensor(addr_data_pair_normal_video,
		   sizeof(addr_data_pair_normal_video) / sizeof(kal_uint16));
}

static void hs_video_setting(void)
{
	ov8856_table_write_cmos_sensor(addr_data_pair_hs_video,
		   sizeof(addr_data_pair_hs_video) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
    ov8856_table_write_cmos_sensor(addr_data_pair_slim_video,
...
{
    ov8856_table_write_cmos_sensor(addr_data_pair_custom1,
		   ...
		   (read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				//if ((read_cmos_sensor(0x302A)) == 0XB0) {
					...
					printk("i2c write id: 0x%x, sensor id: 0x%x, ver = %d<0=r2a,1=r1a>\n",imgsensor.i2c_write_id,*sensor_id, ov8856version);
					vivo_main3_otp_read_when_power_on_ov8856 = vivo_main3_otp_read_ov8856();
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  MAIN3_OTP_ERROR_CODE_OV8856:%d \n", *feature_data,MAIN3_OTP_ERROR_CODE_OV8856);
			switch (*feature_data) {
				...
				if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = MAIN3_OTP_ERROR_CODE_OV8856;//otp_state
