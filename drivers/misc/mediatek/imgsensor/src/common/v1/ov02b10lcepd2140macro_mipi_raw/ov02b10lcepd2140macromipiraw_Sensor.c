...
#include "ov02b10lcepd2140macromipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B10LCEPD2140MACRO_SENSOR_ID,
...
case IMAGE_NORMAL:
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x12, 0x00); //No mirror And Flip
	break;
case IMAGE_H_MIRROR:
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x12, 0x02); //flip
	break;
case IMAGE_V_MIRROR:
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x12, 0x01); //mirror
		break;
case IMAGE_HV_MIRROR:
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x12, 0x03); //Mirror And Flip
	break;
default:
...
UINT32 OV02B10LCEPD2140MACRO_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      OV02B10LCEPD2140MACRO_MIPI_RAW_SensorInit      */
