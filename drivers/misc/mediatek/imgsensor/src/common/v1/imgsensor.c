...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
    ...
    case SENSOR_FEATURE_GET_CTID:
		...
		break;
	case SENSOR_FEATURE_GET_SECOND_MATERIAL:
		if (copy_from_user(
		    &isMainSecondMatFlag,
		    (MUINT8*) pFeatureCtrl->pFeaturePara,
		    FeatureParaLen)) {
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
static ssize_t debug_info_set(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	if (sscanf(buf, "%c%c", &debug_info[0], &debug_info[1]) == 2) {
		debug_info[2] = '\0';
		pr_debug("set imgsensor debug_info success: %s ", debug_info);
	} else {
		debug_info[0] = '\0';
		pr_debug("set imgsensor debug_info fail!");
	}
	return count;
}

static ssize_t debug_info_get(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
...
#endif
#if defined(CONFIG_MTK_CAM_PD1987)
static void imgsensor_GPIO173(void)
{
	struct device_node *of_node = of_find_compatible_node(NULL, NULL, "mediatek,camera_hw");
	if(!of_node)
		pr_err("of_node for GPIO173 is Null.\n");
	GPIO173 = of_get_named_gpio(of_node, "gpio173", 0);	
	pr_err("Get GPIO173 pin=%d.\n", GPIO173);
}
#endif
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX) || defined(CONFIG_MTK_CAM_PD2213F_EX) || defined(CONFIG_MTK_CAM_PD2140NF_EX)
	sysfs_create_file(&pdev->dev.kobj, &dev_attr_IMGSENSOR_DEBUG.attr);
#endif
