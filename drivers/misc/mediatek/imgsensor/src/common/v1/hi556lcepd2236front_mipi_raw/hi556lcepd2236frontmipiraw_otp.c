...
#include "hi556lcepd2236frontmipiraw_Sensor.h"

#define PFX "hi556lcepd2236front_EEPROM_OTP"
...
#define AWB_GROUP_FLAG_ADDR               (0x0426 - UNUSED_DATA_LENGTH)
...
#define INFO_GROUP_FLAG_ADDR              (0x0401 - UNUSED_DATA_LENGTH)
...
#define LSC_GROUP_FLAG_ADDR               (0x045A - UNUSED_DATA_LENGTH)
...
#define SN_GROUP_FLAG_ADDR                (0x1A42 - UNUSED_DATA_LENGTH)
#define SN_HEAD_ADDR_GROUP_1             (0x1A43 - UNUSED_DATA_LENGTH)
#define SN_CHECKSUM_ADDR_GROUP_1         (0x1A4F - UNUSED_DATA_LENGTH)

#define MEATRAIL_VALUE_LENGTH            7
#define MEATRAIL_GROUP_FLAG_ADDR          (0x1A6A - UNUSED_DATA_LENGTH)
#define MEATRAIL_HEAD_ADDR_GROUP_1       (0x1A6B- UNUSED_DATA_LENGTH)
#define MEATRAIL_CHECKSUM_ADDR_GROUP_1   (0x1A72 - UNUSED_DATA_LENGTH)



#define MAX_MEM_BLOCK_SIZE              (INFO_VALUE_LENGTH+AWB_VALUE_LENGTH+LSC_VALUE_LENGTH+SN_VALUE_LENGTH+MEATRAIL_VALUE_LENGTH+5+5)
#define MAX_GROUP_NUM                 3

static unsigned char otp_data_kernel_read_hi556lcepd2236front[MAX_MEM_BLOCK_SIZE*MAX_GROUP_NUM];
unsigned char otp_data_vendor_read_hi556lcepd2236front[INFO_VALUE_LENGTH+AWB_VALUE_LENGTH+LSC_VALUE_LENGTH+SN_VALUE_LENGTH+MEATRAIL_VALUE_LENGTH+2+2];
/**************/
/****vivo cfx add start****/
...
#define VIVO_SUB_VENDOR_LENS_ID 0x29
...
otp_error_code_t SUB_OTP_ERROR_CODE_HI556PD2236 = OTP_ERROR_CODE_NORMAL;
...
static int HI556PD2236_otp_read_setting_init_continuous_read(kal_uint16 addr)
{
    kal_uint32 i;
    ...
    for (i = 0; i < MAX_MEM_BLOCK_SIZE*MAX_GROUP_NUM ; i++) {
	otp_data_kernel_read_hi556lcepd2236front[i] = read_cmos_sensor(0x108);
    }
...
typedef struct OTPMapUnit {
    ...
    kal_uint32 GroupFlagAddr;
    kal_uint32 DataHeadAddr;
    kal_uint32 CheckSumAddrGroup1;
} OTPMapUnit;

kal_uint32 HI556_GROUP_FLAG_VALUE[3] = {0x01, 0x13, 0x37};

OTPMapUnit OtpMapInfo[OTP_TYPE_MAX] = {
    {INFO_VALUE_LENGTH,      INFO_GROUP_FLAG_ADDR,  INFO_HEAD_ADDR_GROUP_1, INFO_CHECKSUM_ADDR_GROUP_1 },
    {AWB_VALUE_LENGTH,       AWB_GROUP_FLAG_ADDR,   AWB_HEAD_ADDR_GROUP_1,  AWB_CHECKSUM_ADDR_GROUP_1  },
    {LSC_VALUE_LENGTH,       LSC_GROUP_FLAG_ADDR,   LSC_HEAD_ADDR_GROUP_1,  LSC_CHECKSUM_ADDR_GROUP_1  },
    {SN_VALUE_LENGTH,        SN_GROUP_FLAG_ADDR,    SN_HEAD_ADDR_GROUP_1,   SN_CHECKSUM_ADDR_GROUP_1   },
    {MEATRAIL_VALUE_LENGTH,  MEATRAIL_GROUP_FLAG_ADDR,    MEATRAIL_HEAD_ADDR_GROUP_1,   MEATRAIL_CHECKSUM_ADDR_GROUP_1   },
};
...
kal_int16 check_checksum_info(OTP_DATA_TYPE ReadOTPType)
{
    ...
    for (GroupIndex = 0; GroupIndex < MAX_GROUP_NUM; GroupIndex++) {
	if (HI556_GROUP_FLAG_VALUE[GroupIndex] == otp_data_kernel_read_hi556lcepd2236front[OtpMapInfo[ReadOTPType].GroupFlagAddr]) {
	...
	for (ReadOffset = 0; ReadOffset < OtpMapInfo[ReadOTPType].ValueLength; ReadOffset++) {
		CaculatedChecksum += otp_data_kernel_read_hi556lcepd2236front[KernelReadHeadAddr + ReadOffset];
	}
	...
	if (CaculatedChecksum == otp_data_kernel_read_hi556lcepd2236front[TmpCheckSumAddr +  GroupIndex * COMMON_CHECKSUM_LENGTH]) {
		IsValid = 1;
		...
		IsValid = 0;
		    ...
		    CaculatedChecksum, otp_data_kernel_read_hi556lcepd2236front[TmpCheckSumAddr +  GroupIndex * 1]);
	break;
	}
    }
    LOG_INF("ReadOTPType is [%d],GroupSelected[%d]", ReadOTPType, GroupIndex+1);
...
kal_uint32 get_otp_single_unit_map(unsigned char *pOutput, OTP_DATA_TYPE ReadOTPType)
{
	kal_uint32 KernelReadHeadAddr = 0x0;
	kal_int16 GroupIndex = 0x0;
    kal_uint32 length = 0;
    ...
    if (GroupIndex) {
	    GroupIndex--;
        length = OtpMapInfo[ReadOTPType].ValueLength; 
	    KernelReadHeadAddr += GroupIndex*(OtpMapInfo[ReadOTPType].ValueLength + COMMON_CHECKSUM_LENGTH);
        if (ReadOTPType == OTP_TYPE_SN) {
            pOutput[0] = 1;//flag;
             memcpy(pOutput + 1, &otp_data_kernel_read_hi556lcepd2236front[KernelReadHeadAddr],
		        OtpMapInfo[ReadOTPType].ValueLength + 1);
            length += 2;
        }else if (ReadOTPType == OTP_TYPE_MATERIAL) {
            pOutput[0] = 1;//flag;
            memcpy(pOutput + 1, &otp_data_kernel_read_hi556lcepd2236front[KernelReadHeadAddr],
		        OtpMapInfo[ReadOTPType].ValueLength + 1);
            length += 2;
        } else {
	        memcpy(pOutput, &otp_data_kernel_read_hi556lcepd2236front[KernelReadHeadAddr],
...
int vivo_sub_otp_read_hi556lcepd2236front(void)
{
    kal_int16 OTPType = 0;
    kal_uint32 result = true;
    kal_uint32 HEADPoint = 0;

    HI556PD2236_otp_read_setting_init_continuous_read(UNUSED_DATA_LENGTH);

    for (OTPType = 0; OTPType < OTP_TYPE_MAX; OTPType++) {
	LOG_INF("OTPType neet to read is [%d]", OTPType);
	result = get_otp_single_unit_map((unsigned char *)&otp_data_vendor_read_hi556lcepd2236front[HEADPoint], (kal_uint32)OTPType);
	HEADPoint += result;
