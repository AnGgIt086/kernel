...
#include "hi556lcepd2216frontmipiraw_Sensor.h"

#define PFX "hi556lcepd2216front_EEPROM_OTP"
...
static unsigned char otp_data_kernel_read_hi556lcepd2216front[MAX_MEM_BLOCK_SIZE*MAX_GROUP_NUM];
unsigned char otp_data_vendor_read_hi556lcepd2216front[INFO_VALUE_LENGTH+AWB_VALUE_LENGTH+LSC_VALUE_LENGTH+SN_VALUE_LENGTH+MEATRAIL_VALUE_LENGTH+2+2];
/**************/
/****vivo cfx add start****/
...
otp_error_code_t SUB_OTP_ERROR_CODE_HI556PD2216 = OTP_ERROR_CODE_NORMAL;
...
static int HI556PD2216_otp_read_setting_init_continuous_read(kal_uint16 addr)
{
    kal_uint32 i;
	...
	otp_data_kernel_read_hi556lcepd2216front[i] = read_cmos_sensor(0x108);
    }
...
kal_int16 check_checksum_info(OTP_DATA_TYPE ReadOTPType)
{
	...
	if (HI556_GROUP_FLAG_VALUE[GroupIndex] == otp_data_kernel_read_hi556lcepd2216front[OtpMapInfo[ReadOTPType].GroupFlagAddr]) {
	...
	for (ReadOffset = 0; ReadOffset < OtpMapInfo[ReadOTPType].ValueLength; ReadOffset++) {
		CaculatedChecksum += otp_data_kernel_read_hi556lcepd2216front[KernelReadHeadAddr + ReadOffset];
	}
	...
	if (CaculatedChecksum == otp_data_kernel_read_hi556lcepd2216front[TmpCheckSumAddr +  GroupIndex * COMMON_CHECKSUM_LENGTH]) {
		IsValid = 1;
		...
		IsValid = 0;
		    ...
		    CaculatedChecksum, otp_data_kernel_read_hi556lcepd2216front[TmpCheckSumAddr +  GroupIndex * 1]);
	break;
	}
    }
    LOG_INF("ReadOTPType is [%d],GroupSelected[%d]", ReadOTPType, GroupIndex+1);
...
kal_uint32 get_otp_single_unit_map(unsigned char *pOutput, OTP_DATA_TYPE ReadOTPType)
{
    ...
    if (GroupIndex) {
        ...
        if (ReadOTPType == OTP_TYPE_SN) {
            pOutput[0] = 1;//flag;
             memcpy(pOutput + 1, &otp_data_kernel_read_hi556lcepd2216front[KernelReadHeadAddr],
	        ...
	        memcpy(pOutput, &otp_data_kernel_read_hi556lcepd2216front[KernelReadHeadAddr],
...
int vivo_sub_otp_read_hi556lcepd2216front(void)
{
    kal_int16 OTPType = 0;
    kal_uint32 result = true;
    kal_uint32 HEADPoint = 0;

    HI556PD2216_otp_read_setting_init_continuous_read(UNUSED_DATA_LENGTH);

    for (OTPType = 0; OTPType < OTP_TYPE_MAX; OTPType++) {
	LOG_INF("OTPType neet to read is [%d]", OTPType);
	result = get_otp_single_unit_map((unsigned char *)&otp_data_vendor_read_hi556lcepd2216front[HEADPoint], (kal_uint32)OTPType);
	HEADPoint += result;
