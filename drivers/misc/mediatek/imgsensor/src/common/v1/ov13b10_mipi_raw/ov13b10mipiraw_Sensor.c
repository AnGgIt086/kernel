...
//	capture_setting(currefps);
	pr_debug("custom1_setting() start! \n");
	write_cmos_sensor_8(0x0100, 0x00);
	...
	write_cmos_sensor_8(0x380f, 0x80);
...
{
//	capture_setting(currefps);
	pr_debug("custom2_setting() start! \n");
	write_cmos_sensor_8(0x0100, 0x00);
	write_cmos_sensor_8(0x0100, 0x00);
	write_cmos_sensor_8(0x0305, 0x46);
	write_cmos_sensor_8(0x3501, 0x0c);
...
static kal_uint32 open(void)
{
    ...
    pr_debug("PLATFORM:MT6765,MIPI 4LANE +++++ ++++ \n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			pr_debug("if before i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
