...
#if defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
		if (ppower_sequence == sensor_power_sequence) {
			if (pd2216gc2375_flg == 1 && ((sensor_idx == 0) || (sensor_idx == 1)) && pwr_status == IMGSENSOR_HW_POWER_STATUS_ON) {
				pr_info("PD2216 zhuhl POWER_STATUS_ON sensor_id %d, pd2216gc2375_flg= %d \n", sensor_idx, pd2216gc2375_flg);
				phw->pdev[1]->set(
...
#if defined(CONFIG_MTK_CAM_PD2216F_EX) || defined(CONFIG_MTK_CAM_PD2216JF_EX)
		if (pd2216gc2375_flg == 1  && ((sensor_idx == 0) || (sensor_idx == 1))) {
			pr_info("PD2216 zhuhl POWER_STATUS_OFF sensor_id %d, pd2216gc2375_flg= %d \n", sensor_idx, pd2216gc2375_flg);
			phw->pdev[1]->set(
			phw->pdev[1]->pinstance,
			2,
			IMGSENSOR_HW_PIN_RST,
