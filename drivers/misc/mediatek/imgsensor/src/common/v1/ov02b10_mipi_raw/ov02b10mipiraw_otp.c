...
int ov02b10_otp_read(void)
{
...
#if 0
	if (is_atboot == 1) {
		printk("[zxw++]AT mode skip ov02b10_otp_read\n");
		return 1;
	}
#endif
	/* guojunzheng add end */
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		/****ModuleInfo****/
		if ((VIVO_VENDOR_SUNWIN != vivo_otp_data_ov02b10[0x01]) && (VIVO_VENDOR_SHINE != vivo_otp_data_ov02b10[0x01])) {
			...
			return 0;
		} else if ((0x10 != vivo_otp_data_ov02b10[0x08]) || 
