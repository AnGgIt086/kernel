...
static int ion_mm_pools_total(struct ion_system_heap *heap)
{
	int i = 0;
	int count = 0;
	struct ion_page_pool *pool;
	...
	for (i  = 0; i < num_orders; i++) {
		count += (ion_mm_pool_total(heap, orders[i], true) << orders[i]);
		count += (ion_mm_pool_total(heap, orders[i], false) << orders[i]);
	}
