...
unsigned int bsp_test_mode __attribute__((weak)) = 1;
	...
	if (aee_pressed_keys == ((1 << AEE_VOLUMEUP_BIT)|(1 << AEE_VOLUMEDOWN_BIT))) {
		...
		if (aee_timer_started) {
			...
			if (hrtimer_cancel(&aee_timer))
				pr_err("try to cancel hrtimer\n");
			pr_err("aee_timer canceled\n");
		}
	}
}
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16U; j++) {
			...
			kpd_aee_handler(keycode, pressed);
			
			input_report_key(keypad->input_dev, keycode, pressed);
			input_sync(keypad->input_dev);
			pr_err("report Linux keycode = %d\n", keycode);
		}
	}
