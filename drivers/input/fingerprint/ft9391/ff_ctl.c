...
static int __init ff_ctl_driver_init(void)
{
	...
	if (node) {
		if (!of_property_read_string(node, "vivo,project-name", &project_name)) {
			if (!strncmp(project_name, "PD2140F_EX", 10)) {
				board_version = get_board_version();
				if (board_version) {
					if (*(board_version + 5) == '0') {
						FF_LOGI("%s, not support fingerprint,do not load focal driver", __func__);
						return err;
					}
					FF_LOGI("%s, support fingerprint, continue following process.", __func__);
				} else {
					FF_LOGI("%s, board_version is NULL.", __func__);
					return err;
				}
			}
		} else {
			FF_LOGI("%s, can not find project-name", __func__);
		}
