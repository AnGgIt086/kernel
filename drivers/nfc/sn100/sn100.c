...
struct nqx_platform_data {
	...
	/*vivo wangxingcheng add begin,for PD2139F_EX nfc_ven wrong pin*/
	unsigned int pd2139_bad_ven_gpio;
	/*vivo wangxingcheng add end*/
	/* vivo zhangjiebin add begin, for VDDIO control*/
...
/*wangxingcheng add begin for PD2139F_EX special*/
static unsigned int pd2139_bad_ven_ctrl;
/*wangxingcheng add end */
...
static int nqx_ese_pwr(struct nqx_dev *nqx_dev, unsigned long int arg)
{
	...
	if (arg == 0) {
		...
		if (!nqx_dev->nfc_ven_enabled) {
			...
			/*wangxingcheng add begin for PD2139F_EX special*/
			if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
				gpio_set_value(nqx_dev->pd2139_bad_ven_gpio, 1);
			}
			/*wangxingcheng add end*/
		...
		if (!nqx_dev->nfc_ven_enabled) {
			...
			/*wangxingcheng add begin for PD2139F_EX special*/
			if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
				gpio_set_value(nqx_dev->pd2139_bad_ven_gpio, 0);
			}
			/*wangxingcheng add end*/
		}
...
static int nfcc_hw_check(struct i2c_client *client, struct nqx_dev *nqx_dev)
{
	...
	unsigned int pd2139_bad_enable_gpio = nqx_dev->pd2139_bad_ven_gpio;
	...
	/*wangxingcheng add begin for PD2139F_EX special*/
	if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
		gpio_set_value(pd2139_bad_enable_gpio, 0);/* ULPM: Disable */
	}
	/*wangxingcheng add end*/
	/* hardware dependent delay */
	msleep(10);
	...
	/*wangxingcheng add begin for PD2139F_EX special*/
	if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
		gpio_set_value(pd2139_bad_enable_gpio, 1);/* ULPM: Disable */
	}
	/*wangxingcheng add end*/
	/* hardware dependent delay */
	msleep(10);
...
static int nfcc_hw_detect(struct nqx_dev *nqx_dev)
{
	...
	do {
		...
		/*wangxingcheng add begin for PD2139F_EX special*/
		if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
			gpio_set_value(pd2139_bad_enable_gpio, 0);
		}
		/*wangxingcheng add end*/
		/* hardware dependent delay */
		msleep(10);
		...
		/*wangxingcheng add begin for PD2139F_EX special*/
		if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
			gpio_set_value(pd2139_bad_enable_gpio, 1);
		}
		/*wangxingcheng add end*/
		msleep(10);
		ret = i2c_master_send(nqx_dev->client, val, 0);
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
	...
	/*wangxingcheng add begin for PD2139F_EX special*/
	if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
		pdata->pd2139_bad_ven_gpio = of_get_named_gpio(np, "mediatek,nq-pd2139_bad_ven", 0);
		if (!gpio_is_valid(pdata->pd2139_bad_ven_gpio)) {
			return -EINVAL;
		}
		pd2139_bad_ven_ctrl = pdata->pd2139_bad_ven_gpio;
	}
	/*wangxingcheng add end*/
			...
			const struct i2c_device_id *id)
{
	...
	/*wangxingcheng add begin for PD2139F_EX special*/
	if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
		if (gpio_is_valid(platform_data->pd2139_bad_ven_gpio)) {
			r = gpio_request(platform_data->pd2139_bad_ven_gpio, "nfc_reset1_gpio");
			if (r) {
				dev_err(&client->dev, "%s: unable to request nfc reset1 gpio [%d]\n", __func__, platform_data->pd2139_bad_ven_gpio);
				goto err_mem;
			}
			gpio_direction_output(platform_data->pd2139_bad_ven_gpio, 0);
			if (r) {
				dev_err(&client->dev, "%s: unable to set direction for nfc reset1 gpio [%d]\n", __func__, platform_data->pd2139_bad_ven_gpio);
				goto err_pd2139_bad_ven_gpio;
			}
		} else {
			dev_err(&client->dev, "%s: nfc reset1 gpio not provided\n", __func__);
			goto err_mem;
		}
	}
	/*wangxingcheng add end*/
	...
	/* vivo wangxingcheng add begin, for pd2139 nfc_ven wrong pin*/
	nqx_dev->pd2139_bad_ven_gpio = platform_data->pd2139_bad_ven_gpio;
	/* vivo wangxingcheng add end*/
	/* vivo zhangjiebin add begin, for VDDIO control*/
	#ifdef VDDIO_USE_LDO_CONTROL
	...
	if (r) {
	    ...
	    /* vivo wangxingcheng add begin , for pd2139 wrong pin*/
		if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
			gpio_set_value(platform_data->pd2139_bad_ven_gpio, 0);
		}
		/* vivo wangxingcheng add end */
	...
	if ((get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) || (get_boot_mode() == RECOVERY_BOOT)) {
		...
		/* vivo wangxingcheng add begin, for pd2139 nfc_ven wrong pin*/
		if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
			r = gpio_direction_output(platform_data->pd2139_bad_ven_gpio, 1);
		}
		/* vivo wangxingcheng add end*/
	} else {
		r = gpio_direction_output(platform_data->en_gpio, 0);
		/* vivo wangxingcheng add begin, for pd2139 nfc_ven wrong pin*/
		if (!strncmp(vivo_project_name, "PD2139F_EX", 10)) {
			r = gpio_direction_output(platform_data->pd2139_bad_ven_gpio, 0);
		}
		/* vivo wangxingcheng add end*/
	}
#endif
...
/*wangxingcheng add begin, for pd2139 wrong nfc_ven pin*/
err_pd2139_bad_ven_gpio:
	gpio_free(platform_data->pd2139_bad_ven_gpio);
/*wangxingcheng add end*/
err_en_gpio:
...
static int nqx_remove(struct i2c_client *client)
{
	...
	/*wangxingcheng add begin, for pd2139 wrong nfc_ven pin*/
	gpio_free(nqx_dev->pd2139_bad_ven_gpio);
	/*wangxingcheng add end*/
